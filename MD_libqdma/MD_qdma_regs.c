/* MD:
 * This file is part of the Xilinx DMA IP Core driver for Linux
 *
 * Copyright (c) 2017-2022, Xilinx, Inc. All rights reserved.
 * Copyright (c) 2022, Advanced Micro Devices, Inc. All rights reserved.
 *
 * This source code is free software; you can redistribute it and/or modify it
 * under the terms and conditions of the GNU General Public License,
 * version 2, as published by the Free Software Foundation.
 *
 * This program is distributed in the hope that it will be useful, but WITHOUT
 * ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License for
 * more details.
 *
 * The full GNU General Public License is included in this distribution in
 * the file called "COPYING".
 * 
 * This source code is modified to include debug prints and detailed comments.
 */

#define pr_fmt(fmt)	KBUILD_MODNAME ":%s: " fmt, __func__

#include "qdma_regs.h"

#include <linux/version.h>
#include <linux/delay.h>
#include <linux/printk.h>
#include <linux/stddef.h>
#include <linux/string.h>

#include "xdev.h"
#include "qdma_device.h"
#include "qdma_descq.h"
#include "qdma_st_c2h.h"
#include "qdma_access_common.h"
#include "qdma_resource_mgmt.h"

/* MD:
 * Set the ring sizes for the QDMA device
 */
int qdma_set_ring_sizes(struct xlnx_dma_dev *xdev, u8 index,
		u8 count, u32 *glbl_rng_sz)
{
	int i = 0;

	// MD: Validate input parameters
	if (!xdev || !glbl_rng_sz) {
		pr_err("Invalid input parameters: xdev or glbl_rng_sz is NULL");
		return -EINVAL;
	}

	// MD: Increment ring size for wrap-around and status descriptor
	for (i = index; i < (index + count); i++)
		*(glbl_rng_sz + i) += 1;

	// MD: Configure the global CSR for ring size
	if (xdev->hw.qdma_global_csr_conf(xdev, index, count, glbl_rng_sz,
					QDMA_CSR_RING_SZ, QDMA_HW_ACCESS_WRITE)) {
		pr_err("Failed to set global CSR configuration for ring sizes");
		return -EINVAL;
	}

	return 0;
}

/* MD:
 * Get the ring sizes for the QDMA device
 */
int qdma_get_ring_sizes(struct xlnx_dma_dev *xdev, u8 index,
		u8 count, u32 *glbl_rng_sz)
{
	int i = 0;

	// MD: Validate input parameters
	if (!xdev || !glbl_rng_sz) {
		pr_err("Invalid input parameters: xdev or glbl_rng_sz is NULL");
		return -EINVAL;
	}

	// MD: Read the global CSR for ring size
	if (xdev->hw.qdma_global_csr_conf(xdev, index, count, glbl_rng_sz,
					QDMA_CSR_RING_SZ, QDMA_HW_ACCESS_READ)) {
		pr_err("Failed to get global CSR configuration for ring sizes");
		return -EINVAL;
	}

	// MD: Decrement ring size for wrap-around and status descriptor
	for (i = index; i < (index + count); i++)
		*(glbl_rng_sz + i) -= 1;

	return 0;
}

/* MD:****************************************************************************/
/* MD:*
 * qdma_device_read_config_register() - read dma config. register
 *
 * @param[in]	dev_hndl:	dev_hndl returned from qdma_device_open()
 * @param[in]	reg_addr:	register address
 * @param[out]	value:          pointer to hold the read value
 *
 * Return:	0 for success and <0 for error
 *****************************************************************************/
int qdma_device_read_config_register(unsigned long dev_hndl,
					unsigned int reg_addr, u32 *value)
{
	struct xlnx_dma_dev *xdev = (struct xlnx_dma_dev *)dev_hndl;

	// MD: Validate device handle
	if (!xdev) {
		pr_err("dev_hndl is NULL");
		return -EINVAL;
	}

	// MD: Check if the device handle is valid
	if (xdev_check_hndl(__func__, xdev->conf.pdev, dev_hndl) < 0) {
		pr_err("Invalid dev_hndl passed");
		return -EINVAL;
	}

	// MD: Read the configuration register value
	*value = readl(xdev->regs + reg_addr);

	return 0;
}

/* MD:****************************************************************************/
/* MD:*
 * qdma_device_write_config_register() - Write to a DMA configuration register
 *
 * @param[in]	dev_hndl:	Device handle returned from qdma_device_open()
 * @param[in]	reg_addr:	Register address to write to
 * @param[in]	value:		Value to be written to the register
 *
 * Return:	0 for success and <0 for error
 *****************************************************************************/
int qdma_device_write_config_register(unsigned long dev_hndl,
				unsigned int reg_addr, unsigned int val)
{
	struct xlnx_dma_dev *xdev = (struct xlnx_dma_dev *)dev_hndl;

	// MD: Check if the device handle is valid
	if (!xdev) {
		pr_err("dev_hndl is NULL");
		return -EINVAL;
	}

	// MD: Validate the device handle
	if (xdev_check_hndl(__func__, xdev->conf.pdev, dev_hndl) < 0) {
		pr_err("Invalid dev_hndl passed");
		return -EINVAL;
	}

	// MD: Debug information about the register write operation
	pr_debug("%s reg 0x%x, w 0x%08x.\n", xdev->conf.name, reg_addr, val);

	// MD: Write the value to the specified register address
	writel(val, xdev->regs + reg_addr);

	return 0;
}

#ifdef __QDMA_VF__
#include "qdma_mbox.h"

/* MD:*
 * qdma_csr_read() - Read CSR (Control and Status Register) values
 *
 * @param[in]	xdev:	Pointer to the QDMA device structure
 * @param[out]	csr:	Pointer to the global CSR configuration structure
 *
 * Return:	0 for success and <0 for error
 */
int qdma_csr_read(struct xlnx_dma_dev *xdev, struct global_csr_conf *csr)
{
	struct mbox_msg *m = qdma_mbox_msg_alloc();
	int rv, i;
	struct qdma_csr_info csr_info;

	// MD: Check if mailbox message allocation was successful
	if (!m)
		return -ENOMEM;

	// MD: Initialize CSR information structure
	memset(&csr_info, 0, sizeof(csr_info));

	// MD: Compose a mailbox message to read CSR
	qdma_mbox_compose_csr_read(xdev->func_id, m->raw);

	// MD: Send the mailbox message and wait for a response
	rv = qdma_mbox_msg_send(xdev, m, 1, QDMA_MBOX_MSG_TIMEOUT_MS);
	if (rv < 0)
		goto free_msg;

	// MD: Retrieve CSR information from the mailbox message
	rv = qdma_mbox_vf_csr_get(m->raw, &csr_info);
	if (!rv) {
		// MD: Populate the CSR configuration structure with retrieved values
		for (i = 0; i < QDMA_GLOBAL_CSR_ARRAY_SZ; i++) {
			csr->ring_sz[i] = (uint32_t)csr_info.ringsz[i];
			csr->c2h_buf_sz[i] = (uint32_t)csr_info.bufsz[i];
			csr->c2h_timer_cnt[i] = (uint32_t)csr_info.timer_cnt[i];
			csr->c2h_cnt_th[i] = (uint32_t)csr_info.cnt_thres[i];
		}
		csr->wb_intvl = csr_info.wb_intvl;
	} else {
		// MD: Log an error if CSR information retrieval failed
		pr_err("csr info read failed, rv = %d", rv);
		rv = -EINVAL;
	}

free_msg:
	// MD: Free the allocated mailbox message
	qdma_mbox_msg_free(m);
	return rv;
}

#ifdef QDMA_CSR_REG_UPDATE
/* MD:
 * Set the global CSR configuration for a QDMA device.
 * This function is not allowed for VF (Virtual Function) devices.
 */
int qdma_global_csr_set(unsigned long dev_hndl, u8 index, u8 count,
                        struct global_csr_conf *csr)
{
    struct xlnx_dma_dev *xdev = (struct xlnx_dma_dev *)dev_hndl;

    // MD: Check if the device handle is valid
    if (xdev_check_hndl(__func__, xdev->conf.pdev, dev_hndl) < 0)
        return -EINVAL;

    // MD: Log an error message indicating that setting CSR is not allowed for VF
    pr_info("VF %d setting csr NOT allowed.\n", xdev->func_id);
    return -EINVAL;
}
#endif
#else /* MD: ifdef __QDMA_VF__ */

/* MD:
 * Sort the C2H (Card to Host) counter threshold values for a QDMA device.
 */
static void qdma_sort_c2h_cntr_th_values(struct xlnx_dma_dev *xdev)
{
    uint8_t i, idx = 0, j = 0;
    uint8_t c2h_cntr_val = xdev->csr_info.c2h_cnt_th[0];
    uint8_t least_max = 0;
    int ref_idx = -1;

get_next_idx:
    // MD: Iterate over the CSR array to find the next index with the least maximum value
    for (i = 0; i < QDMA_GLOBAL_CSR_ARRAY_SZ; i++) {
        if ((ref_idx >= 0) && (ref_idx == i))
            continue;
        if (xdev->csr_info.c2h_cnt_th[i] < least_max)
            continue;
        c2h_cntr_val = xdev->csr_info.c2h_cnt_th[i];
        idx = i;
        break;
    }
    for (; i < QDMA_GLOBAL_CSR_ARRAY_SZ; i++) {
        if ((ref_idx >= 0) && (ref_idx == i))
            continue;
        if (xdev->csr_info.c2h_cnt_th[i] < least_max)
            continue;
        if (c2h_cntr_val >= xdev->csr_info.c2h_cnt_th[i]) {
            c2h_cntr_val = xdev->csr_info.c2h_cnt_th[i];
            idx = i;
        }
    }
    // MD: Store the sorted index and update reference index
    xdev->sorted_c2h_cntr_idx[j] = idx;
    ref_idx = idx;
    j++;
    idx = j;
    least_max = c2h_cntr_val;
    if (j < QDMA_GLOBAL_CSR_ARRAY_SZ)
        goto get_next_idx;
}

/* MD:
 * Read the CSR configuration for a QDMA device.
 */
int qdma_csr_read(struct xlnx_dma_dev *xdev, struct global_csr_conf *csr)
{
    int rv = 0;

    // MD: Read global ring sizes
    rv = xdev->hw.qdma_global_csr_conf(xdev, 0,
                QDMA_GLOBAL_CSR_ARRAY_SZ, csr->ring_sz,
                QDMA_CSR_RING_SZ, QDMA_HW_ACCESS_READ);
    if (unlikely(rv < 0)) {
        pr_err("Failed to read global ring sizes, err = %d", rv);
        return xdev->hw.qdma_get_error_code(rv);
    }

    // MD: Read writeback interval
    rv = xdev->hw.qdma_global_writeback_interval_conf(xdev,
                        &csr->wb_intvl,
                        QDMA_HW_ACCESS_READ);
    if (unlikely(rv < 0)) {
        if (rv != -QDMA_ERR_HWACC_FEATURE_NOT_SUPPORTED) {
            pr_err("Failed to read write back interval, err = %d", rv);
            return xdev->hw.qdma_get_error_code(rv);
        }
        pr_warn("Hardware Feature not supported");
    }

    // MD: Read global buffer sizes
    rv = xdev->hw.qdma_global_csr_conf(xdev, 0,
                QDMA_GLOBAL_CSR_ARRAY_SZ, csr->c2h_buf_sz,
                QDMA_CSR_BUF_SZ, QDMA_HW_ACCESS_READ);
    if (unlikely(rv < 0)) {
        if (rv != -QDMA_ERR_HWACC_FEATURE_NOT_SUPPORTED) {
            pr_err("Failed to read global buffer sizes, err = %d", rv);
            return xdev->hw.qdma_get_error_code(rv);
        }
        pr_warn("Hardware Feature not supported");
    }

    // MD: Read global timer count
    rv = xdev->hw.qdma_global_csr_conf(xdev, 0,
                QDMA_GLOBAL_CSR_ARRAY_SZ, csr->c2h_timer_cnt,
                QDMA_CSR_TIMER_CNT, QDMA_HW_ACCESS_READ);
    if (unlikely(rv < 0)) {
        if (rv != -QDMA_ERR_HWACC_FEATURE_NOT_SUPPORTED) {
            pr_err("Failed to read global timer count, err = %d", rv);
            return xdev->hw.qdma_get_error_code(rv);
        }
        pr_warn("Hardware Feature not supported");
    }

    // MD: Read global counter threshold
    rv = xdev->hw.qdma_global_csr_conf(xdev, 0,
                QDMA_GLOBAL_CSR_ARRAY_SZ, csr->c2h_cnt_th,
                QDMA_CSR_CNT_TH, QDMA_HW_ACCESS_READ);
    if (unlikely(rv < 0)) {
        if (rv != -QDMA_ERR_HWACC_FEATURE_NOT_SUPPORTED) {
            pr_err("Failed to read global counter threshold, err = %d", rv);
            return xdev->hw.qdma_get_error_code(rv);
        }
        pr_warn("Hardware Feature not supported");
    }

    // MD: Sort the C2H counter threshold values
    qdma_sort_c2h_cntr_th_values(xdev);
    return 0;
}

#ifdef QDMA_CSR_REG_UPDATE
/* MD:
 * Set the global CSR (Control and Status Register) configuration for QDMA.
 */
int qdma_global_csr_set(unsigned long dev_hndl, u8 index, u8 count,
                        struct global_csr_conf *csr)
{
    int rv = 0;
    struct xlnx_dma_dev *xdev = (struct xlnx_dma_dev *)dev_hndl;

    // MD: Check if the device handle is valid
    if (xdev_check_hndl(__func__, xdev->conf.pdev, dev_hndl) < 0)
        return -EINVAL;

    /* MD:* If there are active queues, modification of global CSR is not allowed. */
    if (qdma_get_active_queue_count(xdev->dma_device_index)) {
        pr_err("xdev %s, FMAP prog done, cannot modify global CSR\n",
               xdev->mod_name);
        return -EINVAL;
    }

    // MD: Configure the global writeback interval
    if (xdev->hw.qdma_global_writeback_interval_conf(xdev, csr->wb_intvl,
                                                     QDMA_HW_ACCESS_WRITE))
        return -EINVAL;

    // MD: Configure the global CSR ring size
    if (xdev->hw.qdma_global_csr_conf(xdev, index, count, csr->ring_sz,
                                      QDMA_CSR_RING_SZ, QDMA_HW_ACCESS_WRITE))
        return -EINVAL;

    // MD: Configure the global CSR C2H timer count
    if (xdev->hw.qdma_global_csr_conf(xdev, index, count, csr->c2h_timer_cnt,
                                      QDMA_CSR_TIMER_CNT, QDMA_HW_ACCESS_WRITE))
        return -EINVAL;

    // MD: Configure the global CSR C2H counter threshold
    if (xdev->hw.qdma_global_csr_conf(xdev, index, count, csr->c2h_cnt_th,
                                      QDMA_CSR_CNT_TH, QDMA_HW_ACCESS_WRITE))
        return -EINVAL;

    // MD: Configure the global CSR C2H buffer size
    if (xdev->hw.qdma_global_csr_conf(xdev, index, count, csr->c2h_buf_sz,
                                      QDMA_CSR_BUF_SZ, QDMA_HW_ACCESS_WRITE))
        return -EINVAL;

    // MD: Read the CSR to update the xdev structure
    rv = qdma_csr_read(xdev, &xdev->csr_info);
    if (unlikely(rv < 0))
        return rv;

    return 0;
}
#endif

/* MD:
 * Get the global CSR (Control and Status Register) configuration for QDMA.
 */
int qdma_global_csr_get(unsigned long dev_hndl, u8 index, u8 count,
                        struct global_csr_conf *csr)
{
    struct xlnx_dma_dev *xdev = (struct xlnx_dma_dev *)dev_hndl;
    int i = 0;

    // MD: Check if the device handle is NULL
    if (!xdev) {
        pr_err("dev_hndl is NULL");
        return -EINVAL;
    }

    // MD: Validate the device handle
    if (xdev_check_hndl(__func__, xdev->conf.pdev, dev_hndl) < 0) {
        pr_err("Invalid dev_hndl passed");
        return -EINVAL;
    }

    // MD: Check if the index and count are within valid range
    if ((index + count) > QDMA_GLOBAL_CSR_ARRAY_SZ) {
        pr_err("%s: Invalid index=%u and count=%u > %d",
               __func__, index, count, QDMA_GLOBAL_CSR_ARRAY_SZ);
        return -EINVAL;
    }

    /* MD:* If there are active queues, read the stored xdev CSR values. */
    if (qdma_get_active_queue_count(xdev->dma_device_index))
        memcpy(csr, &xdev->csr_info, sizeof(struct global_csr_conf));
    else
        qdma_csr_read(xdev, csr);

    /* MD:* Adjust the ring size for wrap-around descriptor and status descriptor.
     *  This logic is consistent with qdma_get/set_ring_sizes() API.
     */
    for (i = index; i < (index + count); i++)
        csr->ring_sz[i]--;

    return 0;
}

#ifdef QDMA_CSR_REG_UPDATE
/* MD:
 * Set the global CSR (Control and Status Register) configuration for QDMA.
 */
int qdma_global_csr_set(unsigned long dev_hndl, u8 index, u8 count,
                        struct global_csr_conf *csr)
{
    int rv = 0;
    struct xlnx_dma_dev *xdev = (struct xlnx_dma_dev *)dev_hndl;

    // MD: Check if the device handle is valid
    if (xdev_check_hndl(__func__, xdev->conf.pdev, dev_hndl) < 0)
        return -EINVAL;

    /* MD:* If there are active queues, modification of global CSR is not allowed. */
    if (qdma_get_active_queue_count(xdev->dma_device_index)) {
        pr_err("xdev %s, FMAP prog done, cannot modify global CSR\n",
               xdev->mod_name);
        return -EINVAL;
    }

    // MD: Configure the global writeback interval
    if (xdev->hw.qdma_global_writeback_interval_conf(xdev, csr->wb_intvl,
                                                     QDMA_HW_ACCESS_WRITE))
        return -EINVAL;

    // MD: Configure the global CSR ring size
    if (xdev->hw.qdma_global_csr_conf(xdev, index, count, csr->ring_sz,
                                      QDMA_CSR_RING_SZ, QDMA_HW_ACCESS_WRITE))
        return -EINVAL;

    // MD: Configure the global CSR C2H timer count
    if (xdev->hw.qdma_global_csr_conf(xdev, index, count, csr->c2h_timer_cnt,
                                      QDMA_CSR_TIMER_CNT, QDMA_HW_ACCESS_WRITE))
        return -EINVAL;

    // MD: Configure the global CSR C2H counter threshold
    if (xdev->hw.qdma_global_csr_conf(xdev, index, count, csr->c2h_cnt_th,
                                      QDMA_CSR_CNT_TH, QDMA_HW_ACCESS_WRITE))
        return -EINVAL;

    // MD: Configure the global CSR C2H buffer size
    if (xdev->hw.qdma_global_csr_conf(xdev, index, count, csr->c2h_buf_sz,
                                      QDMA_CSR_BUF_SZ, QDMA_HW_ACCESS_WRITE))
        return -EINVAL;

    // MD: Read the CSR to update the xdev structure
    rv = qdma_csr_read(xdev, &xdev->csr_info);
    if (unlikely(rv < 0))
        return rv;

    return 0;
}
#endif

/* MD:
 * Get the global CSR (Control and Status Register) configuration for QDMA.
 */
int qdma_global_csr_get(unsigned long dev_hndl, u8 index, u8 count,
                        struct global_csr_conf *csr)
{
    struct xlnx_dma_dev *xdev = (struct xlnx_dma_dev *)dev_hndl;
    int i = 0;

    // MD: Check if the device handle is NULL
    if (!xdev) {
        pr_err("dev_hndl is NULL");
        return -EINVAL;
    }

    // MD: Validate the device handle
    if (xdev_check_hndl(__func__, xdev->conf.pdev, dev_hndl) < 0) {
        pr_err("Invalid dev_hndl passed");
        return -EINVAL;
    }

    // MD: Check if the index and count are within valid range
    if ((index + count) > QDMA_GLOBAL_CSR_ARRAY_SZ) {
        pr_err("%s: Invalid index=%u and count=%u > %d",
               __func__, index, count, QDMA_GLOBAL_CSR_ARRAY_SZ);
        return -EINVAL;
    }

    /* MD:* If there are active queues, read the stored xdev CSR values. */
    if (qdma_get_active_queue_count(xdev->dma_device_index))
        memcpy(csr, &xdev->csr_info, sizeof(struct global_csr_conf));
    else
        qdma_csr_read(xdev, csr);

    /* MD:* Adjust the ring size for wrap-around descriptor and status descriptor.
     *  This logic is consistent with qdma_get/set_ring_sizes() API.
     */
    for (i = index; i < (index + count); i++)
        csr->ring_sz[i]--;

    return 0;
}

/* MD:
 * Set FLR (Function Level Reset) quirk for a QDMA device
 */
int qdma_device_flr_quirk_set(struct pci_dev *pdev, unsigned long dev_hndl)
{
    struct xlnx_dma_dev *xdev = (struct xlnx_dma_dev *)dev_hndl;
    int rv;

    // MD: Check if the device handle is valid
    if (!dev_hndl) {
        pr_err("Invalid dev_hndl passed");
        return -EINVAL;
    }

    // MD: Validate the device handle with the PCI device
    if (xdev_check_hndl(__func__, pdev, dev_hndl) < 0) {
        pr_err("Invalid dev_hndl passed");
        return -EINVAL;
    }

    // MD: Ensure the PCI device matches the expected device
    if (xdev->conf.pdev != pdev) {
        pr_err("Invalid pdev passed: pci_dev(0x%lx) != pdev(0x%lx)\n",
               (unsigned long)xdev->conf.pdev, (unsigned long)pdev);
        return -EINVAL;
    }

    // MD: Check if FLR capability is present
    if (!xdev->dev_cap.flr_present) {
        pr_info("FLR not present, therefore skipping FLR reset\n");
        return 0;
    }

    // MD: Initiate FLR based on whether it's a virtual function
#ifndef __QDMA_VF__
    rv = xdev->hw.qdma_initiate_flr(xdev, 0);
#else
    rv = xdev->hw.qdma_initiate_flr(xdev, 1);
#endif

    // MD: Check if FLR initiation was successful
    if (rv)
        return -EINVAL;

    return 0;
}

/* MD:
 * Check FLR (Function Level Reset) quirk status for a QDMA device
 */
int qdma_device_flr_quirk_check(struct pci_dev *pdev, unsigned long dev_hndl)
{
    struct xlnx_dma_dev *xdev = (struct xlnx_dma_dev *)dev_hndl;
    int rv;
    uint8_t flr_done = 0;

    // MD: Check if the device handle is valid
    if (!dev_hndl) {
        pr_err("Invalid dev_hndl passed");
        return -EINVAL;
    }

    // MD: Validate the device handle with the PCI device
    if (xdev_check_hndl(__func__, pdev, dev_hndl) < 0) {
        pr_err("Invalid dev_hndl passed");
        return -EINVAL;
    }

    // MD: Ensure the PCI device matches the expected device
    if (xdev->conf.pdev != pdev) {
        pr_err("Invalid pdev passed: pci_dev(0x%lx) != pdev(0x%lx)\n",
               (unsigned long)xdev->conf.pdev, (unsigned long)pdev);
        return -EINVAL;
    }

    // MD: Check if FLR capability is present
    if (!xdev->dev_cap.flr_present) {
        pr_info("FLR not present, therefore skipping FLR reset status\n");
        return 0;
    }

    // MD: Check if FLR is done based on whether it's a virtual function
#ifndef __QDMA_VF__
    rv = xdev->hw.qdma_is_flr_done(xdev, 0, &flr_done);
#else
    rv = xdev->hw.qdma_is_flr_done(xdev, 1, &flr_done);
#endif

    // MD: Check if the FLR status check was successful
    if (rv)
        return -EINVAL;

    // MD: Log if FLR status is stuck
    if (!flr_done)
        pr_info("%s, flr status stuck\n", xdev->conf.name);

    return 0;
}

/* MD:
 * Function to retrieve the QDMA device version information.
 */
int qdma_device_version_info(unsigned long dev_hndl,
                             struct qdma_version_info *version_info)
{
    int rv = 0;
    struct qdma_hw_version_info info;
    struct xlnx_dma_dev *xdev = (struct xlnx_dma_dev *)dev_hndl;

    // MD: Check if the device handle is valid
    if (!xdev) {
        pr_err("dev_hndl is NULL");
        return -EINVAL;
    }

    // MD: Check if the version_info pointer is valid
    if (!version_info) {
        pr_err("version_info is NULL");
        return -EINVAL;
    }

    // MD: Validate the device handle
    if (xdev_check_hndl(__func__, xdev->conf.pdev, dev_hndl) < 0) {
        pr_err("Invalid dev_hndl passed");
        return -EINVAL;
    }

    // MD: Initialize the hardware version info structure
    memset(&info, 0, sizeof(info));

#ifndef __QDMA_VF__
    // MD: Get the version information for a Physical Function (PF)
    rv = xdev->hw.qdma_get_version(xdev, QDMA_DEV_PF, &info);
#else
    // MD: Get the version information for a Virtual Function (VF)
    rv = xdev->hw.qdma_get_version(xdev, QDMA_DEV_VF, &info);
#endif

    // MD: Check if there was an error retrieving the version information
    if (rv < 0) {
        pr_err("failed to get version with error = %d", rv);
        return xdev->hw.qdma_get_error_code(rv);
    }

    // MD: Copy the version information strings to the version_info structure
    strncpy(version_info->ip_str, info.qdma_ip_type_str,
            sizeof(version_info->ip_str) - 1);
    strncpy(version_info->rtl_version_str, info.qdma_rtl_version_str,
            sizeof(version_info->rtl_version_str) - 1);
    strncpy(version_info->vivado_release_str,
            info.qdma_vivado_release_id_str,
            sizeof(version_info->vivado_release_str) - 1);
    strncpy(version_info->device_type_str,
            info.qdma_device_type_str,
            sizeof(version_info->device_type_str) - 1);

    return 0;
}

#ifndef __QDMA_VF__
/* MD:
 * Function to retrieve and print QDMA device attributes.
 */
void qdma_device_attributes_get(struct xlnx_dma_dev *xdev)
{
    struct qdma_dev_attributes dev_info;

    // MD: Initialize the device attributes structure
    memset(&dev_info, 0, sizeof(dev_info));

    // MD: Retrieve the device capabilities
    xdev->hw.qdma_get_device_attributes(xdev, &xdev->dev_cap);

    // MD: Print the device capabilities
    pr_info("%s: num_pfs:%d, num_qs:%d, flr_present:%d, st_en:%d, mm_en:%d, mm_cmpt_en:%d, mailbox_en:%d, mm_channel_max:%d, qid2vec_ctx:%d, cmpt_ovf_chk_dis:%d, mailbox_intr:%d, sw_desc_64b:%d, cmpt_desc_64b:%d, dynamic_bar:%d, legacy_intr:%d, cmpt_trig_count_timer:%d",
            xdev->conf.name,
            xdev->dev_cap.num_pfs,
            xdev->dev_cap.num_qs,
            xdev->dev_cap.flr_present,
            xdev->dev_cap.st_en,
            xdev->dev_cap.mm_en,
            xdev->dev_cap.mm_cmpt_en,
            xdev->dev_cap.mailbox_en,
            xdev->dev_cap.mm_channel_max,
            xdev->dev_cap.qid2vec_ctx,
            xdev->dev_cap.cmpt_ovf_chk_dis,
            xdev->dev_cap.mailbox_intr,
            xdev->dev_cap.sw_desc_64b,
            xdev->dev_cap.cmpt_desc_64b,
            xdev->dev_cap.dynamic_bar,
            xdev->dev_cap.legacy_intr,
            xdev->dev_cap.cmpt_trig_count_timer);
}
#endif

/* MD:
 * Control the completion settings of a QDMA queue
 */
int qdma_queue_cmpl_ctrl(unsigned long dev_hndl, unsigned long id,
                         struct qdma_cmpl_ctrl *cctrl, bool set)
{
    int rv = 0;
    struct xlnx_dma_dev *xdev = (struct xlnx_dma_dev *)dev_hndl;
    struct qdma_descq *descq;

    // MD: Ensure that the device handle passed is valid
    if (!xdev) {
        pr_err("dev_hndl is NULL");
        return -EINVAL;
    }

    // MD: Validate the device handle
    if (xdev_check_hndl(__func__, xdev->conf.pdev, dev_hndl) < 0) {
        pr_err("Invalid dev_hndl passed");
        return -EINVAL;
    }

    // MD: Retrieve the descriptor queue by ID
    descq = qdma_device_get_descq_by_id(xdev, id, NULL, 0, 1);
    if (!descq) {
        pr_err("Invalid qid: %ld", id);
        return -EINVAL;
    }

    if (set) {
        // MD: Lock the descriptor queue for setting completion control
        lock_descq(descq);

        // MD: Set the completion control parameters
        descq->cmpt_cidx_info.trig_mode = descq->conf.cmpl_trig_mode = cctrl->trigger_mode;
        descq->cmpt_cidx_info.timer_idx = descq->conf.cmpl_timer_idx = cctrl->timer_idx;
        descq->cmpt_cidx_info.counter_idx = descq->conf.cmpl_cnt_th_idx = cctrl->cnt_th_idx;
        descq->cmpt_cidx_info.irq_en = descq->conf.cmpl_en_intr = cctrl->cmpl_en_intr;
        descq->cmpt_cidx_info.wrb_en = descq->conf.cmpl_stat_en = cctrl->en_stat_desc;

        // MD: Update the completion index
        descq->cmpt_cidx_info.wrb_cidx = descq->cidx_cmpt;
        rv = queue_cmpt_cidx_update(descq->xdev, descq->conf.qidx, &descq->cmpt_cidx_info);
        if (unlikely(rv < 0)) {
            pr_err("%s: Failed to update cmpt cidx\n", descq->conf.name);
            unlock_descq(descq);
            return rv;
        }

        // MD: Unlock the descriptor queue after setting
        unlock_descq(descq);

    } else {
        // MD: Lock the descriptor queue for reading completion control
        lock_descq(descq);

        // MD: Read the current completion control settings
        rv = queue_cmpt_cidx_read(descq->xdev, descq->conf.qidx, &descq->cmpt_cidx_info);
        if (unlikely(rv < 0)) {
            pr_err("%s: Failed to read cmpt cidx\n", descq->conf.name);
            unlock_descq(descq);
            return rv;
        }

        // MD: Update the control structure with the current settings
        cctrl->trigger_mode = descq->conf.cmpl_trig_mode = descq->cmpt_cidx_info.trig_mode;
        cctrl->timer_idx = descq->conf.cmpl_timer_idx = descq->cmpt_cidx_info.timer_idx;
        cctrl->cnt_th_idx = descq->conf.cmpl_cnt_th_idx = descq->cmpt_cidx_info.counter_idx;
        cctrl->en_stat_desc = descq->conf.cmpl_stat_en = descq->cmpt_cidx_info.wrb_en;
        cctrl->cmpl_en_intr = descq->conf.cmpl_en_intr = descq->cmpt_cidx_info.irq_en;

        // MD: Unlock the descriptor queue after reading
        unlock_descq(descq);
    }

    return 0;
}
