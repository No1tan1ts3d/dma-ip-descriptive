...
// MD: Error code mapping between QDMA and Linux error codes
...
// MD: Mutex for resource management APIs
static DEFINE_MUTEX(res_mutex);

// MD: Allocate zero-initialized memory
void *qdma_calloc(uint32_t num_blocks, uint32_t size)
{
    return kzalloc((num_blocks * (size_t)size), GFP_KERNEL);
}

// MD: Free allocated memory
void qdma_memfree(void *memptr)
{
    kfree(memptr);
}

// MD: Delay execution for a specified number of microseconds
void qdma_udelay(u32 delay_us)
{
    udelay(delay_us);
}

// MD: Lock hardware register access
...
// MD: Unlock hardware register access
...
// MD: Read a value from a hardware register
...
// MD: Write a value to a hardware register
...
// MD: Acquire resource management lock
void qdma_resource_lock_take(void)
{
    mutex_lock(&res_mutex);
}

// MD: Release resource management lock
...
// MD: Copy a string with a specified maximum length
void qdma_strncpy(char *dest, const char *src, size_t n)
{
    strncpy(dest, src, n);
}

// MD: Get the Linux error code corresponding to a QDMA error code
...
// MD: Placeholder for write memory barrier
int qdma_io_wmb(void)
{
     ...
     * pidx/cmpt_cidx updates, whereas Linux driver calls wmb() after
     * pidx/cmpt_cidx updates. As Linux driver performance numbers
     * are good with current changes, so keeping this function
     * as a placeholder for future changes related to memory barriers.
     */
    return 0;
}
