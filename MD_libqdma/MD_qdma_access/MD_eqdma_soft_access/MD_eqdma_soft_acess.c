...
/* MD: H2C Throttle settings for QDMA 4.0 */
...
/* MD: H2C Throttle settings for QDMA 5.0 */
...
/* MD: CSR Default values for QDMA 5.0 */
...
/* MD: C2H prefetch Throttle configuration */
#define EQDMA5_DEFAULT_C2H_EVT_QCNT_TH       0x38
#define EQDMA5_DEFAULT_C2H_PFCH_QCNT         0x3c

/* MD:* Auxiliary Bitmasks for fields spanning multiple words */
...
/* MD: Add debug print statements to track execution flow */
static void eqdma_hw_st_h2c_err_process(void *dev_hndl) {
    printk(KERN_DEBUG "Entering eqdma_hw_st_h2c_err_process\n");
    // MD: Error processing logic for H2C errors
    printk(KERN_DEBUG "Exiting eqdma_hw_st_h2c_err_process\n");
}

static void eqdma_hw_st_c2h_err_process(void *dev_hndl) {
    printk(KERN_DEBUG "Entering eqdma_hw_st_c2h_err_process\n");
    // MD: Error processing logic for C2H errors
    printk(KERN_DEBUG "Exiting eqdma_hw_st_c2h_err_process\n");
}

static void eqdma_hw_desc_err_process(void *dev_hndl) {
    printk(KERN_DEBUG "Entering eqdma_hw_desc_err_process\n");
    // MD: Error processing logic for descriptor errors
    printk(KERN_DEBUG "Exiting eqdma_hw_desc_err_process\n");
}

static void eqdma_hw_trq_err_process(void *dev_hndl) {
    printk(KERN_DEBUG "Entering eqdma_hw_trq_err_process\n");
    // MD: Error processing logic for TRQ errors
    printk(KERN_DEBUG "Exiting eqdma_hw_trq_err_process\n");
}

static void eqdma_hw_ram_sbe_err_process(void *dev_hndl) {
    printk(KERN_DEBUG "Entering eqdma_hw_ram_sbe_err_process\n");
    // MD: Error processing logic for single-bit errors in RAM
    printk(KERN_DEBUG "Exiting eqdma_hw_ram_sbe_err_process\n");
}

static void eqdma_hw_ram_dbe_err_process(void *dev_hndl) {
    printk(KERN_DEBUG "Entering eqdma_hw_ram_dbe_err_process\n");
    // MD: Error processing logic for double-bit errors in RAM
    printk(KERN_DEBUG "Exiting eqdma_hw_ram_dbe_err_process\n");
}

static void eqdma_mm_h2c0_err_process(void *dev_hndl) {
    printk(KERN_DEBUG "Entering eqdma_mm_h2c0_err_process\n");
    // MD: Error processing logic for MM H2C0 errors
    printk(KERN_DEBUG "Exiting eqdma_mm_h2c0_err_process\n");
}

static void eqdma_mm_c2h0_err_process(void *dev_hndl) {
    printk(KERN_DEBUG "Entering eqdma_mm_c2h0_err_process\n");
    // MD: Error processing logic for MM C2H0 errors
    printk(KERN_DEBUG "Exiting eqdma_mm_c2h0_err_process\n");
}

/* MD: Structure to hold error information for EQDMA hardware errors */
static struct eqdma_hw_err_info eqdma_err_info[EQDMA_ERRS_ALL] = {
    /* MD: Descriptor errors */
    {
        EQDMA_DSC_ERR_POISON,  // MD: Error code for poison error
        "Poison error",        // MD: Description of the error
        EQDMA_GLBL_DSC_ERR_MSK_ADDR,  // MD: Address for the global descriptor error mask
        EQDMA_GLBL_DSC_ERR_STS_ADDR,  // MD: Address for the global descriptor error status
        ...
        &eqdma_hw_desc_err_process    // MD: Function pointer to the error processing function
    },
	{
		EQDMA_DSC_ERR_UR_CA,
...
/* MD: Array to hold all EQDMA hardware error codes */
static int32_t all_eqdma_hw_errs[EQDMA_TOTAL_LEAF_ERROR_AGGREGATORS] = {
    EQDMA_DSC_ERR_ALL,       // MD: Descriptor errors
    EQDMA_TRQ_ERR_ALL,       // MD: Transfer Queue errors
    EQDMA_ST_C2H_ERR_ALL,    // MD: Stream C2H errors
    EQDMA_ST_FATAL_ERR_ALL,  // MD: Stream Fatal errors
    EQDMA_ST_H2C_ERR_ALL,    // MD: Stream H2C errors
    EQDMA_SBE_1_ERR_ALL,     // MD: Single-bit error 1
    EQDMA_SBE_ERR_ALL,       // MD: Single-bit errors
    EQDMA_DBE_1_ERR_ALL,     // MD: Double-bit error 1
    EQDMA_DBE_ERR_ALL,       // MD: Double-bit errors
    EQDMA_MM_C2H_ERR_ALL,    // MD: Memory-mapped C2H errors
    EQDMA_MM_H2C0_ERR_ALL    // MD: Memory-mapped H2C0 errors
};

/* MD: Debug print statement to indicate initialization of hardware errors array */
printk(KERN_DEBUG "Initializing all EQDMA hardware errors array\n");

/* MD: Structure to define software context entries for EQDMA */
static struct qctx_entry eqdma_sw_ctxt_entries[] = {
    ...
    {"Write back/Intr Check", 0},         // MD: Writeback/Interrupt Check
    ...
    {"Ring Size", 0},                     // MD: Ring Size
    {"Descriptor Size", 0},               // MD: Descriptor Size
    {"Bypass Enable", 0},                 // MD: Bypass Enable status
    {"MM Channel", 0},                    // MD: Memory-mapped Channel
    ...
    {"Is Memory Mapped", 0},              // MD: Is Memory Mapped status
    ...
    {"Pack descriptor output interface", 0}, // MD: Pack descriptor output interface
    {"Irq Bypass", 0},                    // MD: IRQ Bypass status
};

/* MD: Debug print statement to indicate initialization of software context entries */
printk(KERN_DEBUG "Initializing EQDMA software context entries\n");

/* MD: Structure to define hardware context entries for EQDMA */
static struct qctx_entry eqdma_hw_ctxt_entries[] = {
    {"CIDX", 0},                          // MD: Consumer Index
    {"Credits Consumed", 0},              // MD: Credits Consumed
    {"Descriptors Pending", 0},           // MD: Descriptors Pending
    {"Queue Invalid No Desc Pending", 0}, // MD: Queue Invalid No Descriptor Pending
    {"Eviction Pending", 0},              // MD: Eviction Pending status
    {"Fetch Pending", 0},                 // MD: Fetch Pending status
};

/* MD: Debug print statement to indicate initialization of hardware context entries */
printk(KERN_DEBUG "Initializing EQDMA hardware context entries\n");

/* MD: Structure to define credit context entries for EQDMA */
static struct qctx_entry eqdma_credit_ctxt_entries[] = {
    {"Credit", 0},                        // MD: Credit value
};

/* MD: Debug print statement to indicate initialization of credit context entries */
printk(KERN_DEBUG "Initializing EQDMA credit context entries\n");

/* MD: Structure to define FMAP context entries for EQDMA */
static struct qctx_entry eqdma_fmap_ctxt_entries[] = {
    {"Queue Base", 0},                    // MD: Queue Base address
    {"Queue Max", 0},                     // MD: Maximum Queue value
};

/* MD: Debug print statement to indicate initialization of FMAP context entries */
printk(KERN_DEBUG "Initializing EQDMA FMAP context entries\n");

/* MD: Structure to define completion context entries for EQDMA */
static struct qctx_entry eqdma_cmpt_ctxt_entries[] = {
    ...
    {"Trigger Mode", 0},                  // MD: Trigger Mode
    {"Function Id", 0},                   // MD: Function Identifier
    {"Counter Index", 0},                 // MD: Counter Index
    {"Timer Index", 0},                   // MD: Timer Index
    {"Interrupt State", 0},               // MD: Interrupt State
    {"Color", 0},                         // MD: Color status
    {"Ring Size", 0},                     // MD: Ring Size
    {"Base Addr High (L)[37:6]", 0},      // MD: Base Address High (Low part)
    {"Base Addr High(H)[63:38]", 0},      // MD: Base Address High (High part)
    {"Descriptor Size", 0},               // MD: Descriptor Size
    {"PIDX", 0},                          // MD: Producer Index
    {"CIDX", 0},                          // MD: Consumer Index
    ...
    {"c2h Direction", 0},                 // MD: C2H Direction
    {"Base Addr Low[5:2]", 0},            // MD: Base Address Low
    {"Shared Completion Queue", 0},       // MD: Shared Completion Queue status
};

/* MD: Debug print statement to indicate initialization of completion context entries */
printk(KERN_DEBUG "Initializing EQDMA completion context entries\n");

/* MD: Structure to define C2H prefetch context entries for EQDMA */
static struct qctx_entry eqdma_c2h_pftch_ctxt_entries[] = {
    ...
    {"Software Credit", 0},               // MD: Software Credit value
    {"Valid", 0},                         // MD: Valid status
};

/* MD: Debug print statement to indicate initialization of C2H prefetch context entries */
printk(KERN_DEBUG "Initializing EQDMA C2H prefetch context entries\n");

/* MD: Structure to define indirect interrupt context entries for EQDMA */
static struct qctx_entry eqdma_ind_intr_ctxt_entries[] = {
    {"valid", 0},                         // MD: Valid status
    {"vec", 0},                           // MD: Vector
    {"int_st", 0},                        // MD: Interrupt State
    {"color", 0},                         // MD: Color status
    {"baddr_4k (Low)", 0},                // MD: Base Address 4K (Low part)
    {"baddr_4k (High)", 0},               // MD: Base Address 4K (High part)
...
/* MD: Debug print statement to indicate initialization of indirect interrupt context entries */
printk(KERN_DEBUG "Initializing EQDMA indirect interrupt context entries\n");

#include <linux/kernel.h>  // MD: For printk

/* MD: Function to invalidate an indirect register context */
static int eqdma_indirect_reg_invalidate(void *dev_hndl,
        enum ind_ctxt_cmd_sel sel, uint16_t hw_qid) {
    printk(KERN_DEBUG "Invalidating indirect register context: sel=%d, hw_qid=%d\n", sel, hw_qid);
    // MD: Logic to invalidate the indirect register context
    return 0;  // MD: Return appropriate status
}

/* MD: Function to clear an indirect register context */
static int eqdma_indirect_reg_clear(void *dev_hndl,
        enum ind_ctxt_cmd_sel sel, uint16_t hw_qid) {
    printk(KERN_DEBUG "Clearing indirect register context: sel=%d, hw_qid=%d\n", sel, hw_qid);
    // MD: Logic to clear the indirect register context
    return 0;  // MD: Return appropriate status
}

/* MD: Function to read from an indirect register context */
static int eqdma_indirect_reg_read(void *dev_hndl, enum ind_ctxt_cmd_sel sel,
        uint16_t hw_qid, uint32_t cnt, uint32_t *data) {
    printk(KERN_DEBUG "Reading from indirect register context: sel=%d, hw_qid=%d, cnt=%d\n", sel, hw_qid, cnt);
    // MD: Logic to read from the indirect register context
    return 0;  // MD: Return appropriate status
}

/* MD: Function to write to an indirect register context */
static int eqdma_indirect_reg_write(void *dev_hndl, enum ind_ctxt_cmd_sel sel,
        uint16_t hw_qid, uint32_t *data, uint16_t cnt) {
    printk(KERN_DEBUG "Writing to indirect register context: sel=%d, hw_qid=%d, cnt=%d\n", sel, hw_qid, cnt);
    // MD: Logic to write to the indirect register context
    return 0;  // MD: Return appropriate status
}

/* MD: Function to get the number of configuration registers */
uint32_t eqdma_get_config_num_regs(void) {
    printk(KERN_DEBUG "Getting number of configuration registers\n");
    return eqdma_config_num_regs_get();
}

/* MD: Function to get the configuration registers */
struct xreg_info *eqdma_get_config_regs(void) {
    printk(KERN_DEBUG "Getting configuration registers\n");
    return eqdma_config_regs_get();
}

/* MD: Function to calculate the buffer length for register dump */
uint32_t eqdma_reg_dump_buf_len(void) {
    uint32_t length = (eqdma_config_num_regs_get() + 1) * REG_DUMP_SIZE_PER_LINE;
    printk(KERN_DEBUG "Calculated register dump buffer length: %u\n", length);
    return length;
}

/* MD: Function to calculate the context buffer length */
int eqdma_context_buf_len(uint8_t st, enum qdma_dev_q_type q_type, uint32_t *buflen) {
    int len = 0;
    printk(KERN_DEBUG "Calculating context buffer length for q_type=%d\n", q_type);
    ...
    *buflen = len;
    printk(KERN_DEBUG "Calculated context buffer length: %d\n", len);
    return 0;
}

/* MD: Function to calculate the interrupt context buffer length */
static uint32_t eqdma_intr_context_buf_len(void) {
    ...
    len += (((sizeof(eqdma_ind_intr_ctxt_entries) /
            ...
            REG_DUMP_SIZE_PER_LINE);
    printk(KERN_DEBUG "Calculated interrupt context buffer length: %u\n", len);
    return len;
}

/* MD: Function to set performance optimization options for EQDMA */
...
// MD: Define constants for configuration settings
...
#define EQDMA_GLBL_DSC_CFG_WB_ACC_INT 5
    ...
    // MD: Set global descriptor configuration register
    reg_val =
        FIELD_SET(GLBL_DSC_CFG_RSVD_1_MASK, GLBL_DSC_CFG_RSVD_1_DFLT) |
        FIELD_SET(GLBL_DSC_CFG_C2H_UODSC_LIMIT_MASK, EQDMA_GLBL_DSC_CFG_C2H_UODSC_LIMIT) |
        FIELD_SET(GLBL_DSC_CFG_H2C_UODSC_LIMIT_MASK, EQDMA_GLBL_DSC_CFG_H2C_UODSC_LIMIT) |
        FIELD_SET(GLBL_DSC_CFG_UNC_OVR_COR_MASK, GLBL_DSC_CFG_UNC_OVR_COR_DFLT) |
        FIELD_SET(GLBL_DSC_CFG_CTXT_FER_DIS_MASK, GLBL_DSC_CFG_CTXT_FER_DIS_DFLT) |
...
// MD: Define constants for miscellaneous control settings
...
#define EQDMA_CFG_BLK_MISC_CTL_RQ_METERING_MUL 9
    ...
    // MD: Set miscellaneous control register
    reg_val =
        FIELD_SET(CFG_BLK_MISC_CTL_RSVD_1_MASK, CFG_BLK_MISC_CTL_RSVD_1_DFLT) |
        FIELD_SET(CFG_BLK_MISC_CTL_10B_TAG_EN_MASK, CFG_BLK_MISC_CTL_10B_TAG_DFLT) |
        FIELD_SET(CFG_BLK_MISC_CTL_RSVD_2_MASK, CFG_BLK_MISC_CTL_RSVD_2_DFLT) |
        FIELD_SET(CFG_BLK_MISC_CTL_AXI_WBK_MASK, CFG_BLK_MISC_CTL_AXI_WBK_DFLT) |
        FIELD_SET(CFG_BLK_MISC_CTL_AXI_DSC_MASK, CFG_BLK_MISC_CTL_AXI_DSC_DFLT) |
        ...
        FIELD_SET(CFG_BLK_MISC_CTL_RQ_METERING_MULTIPLIER_MASK, EQDMA_CFG_BLK_MISC_CTL_RQ_METERING_MUL);
...
// MD: Define constants for prefetch configuration
#define EQDMA_PFTCH_CFG_EVT_PFTH_FL_TH 256
#define C2H_PFCH_CFG_FL_TH_DFLT 256

    // MD: Set prefetch configuration register
    reg_val =
...
// MD: Define constants for prefetch cache configuration
...
pfch_cache_dpth = qdma_reg_read(dev_hndl, EQDMA_C2H_PFCH_CACHE_DEPTH_ADDR);

    // MD: Set prefetch cache configuration register
    reg_val =
        FIELD_SET(C2H_PFCH_CFG_1_EVT_QCNT_TH_MASK, (pfch_cache_dpth - 4)) |
...
// MD: Define constants for prefetch configuration 2
...
#define C2H_PFCH_CFG_2_NUM_DFLT 8
    ...
    // MD: Set prefetch configuration 2 register
    reg_val =
        ...
        FIELD_SET(C2H_PFCH_CFG_2_VAR_DESC_NO_DROP_MASK, C2H_PFCH_CFG_2_VAR_DESC_NO_DROP_DFLT) |
        FIELD_SET(C2H_PFCH_CFG_2_LL_SZ_TH_MASK, C2H_PFCH_CFG_2_LL_SZ_TH_DFLT) |
        FIELD_SET(C2H_PFCH_CFG_2_VAR_DESC_NUM_MASK, C2H_PFCH_CFG_2_VAR_DESC_NUM) |
...
// MD: Define constants for prefetch configuration 3
...
#define PFCH_CFG_3_VAR_DESC_LG_PKT_CAM_CN_TH_DFLT 0
    ...
    // MD: Set prefetch configuration 3 register
    reg_val =
        FIELD_SET(PFCH_CFG_3_RSVD_MASK, PFCH_CFG_3_RSVD_DFLT) |
        FIELD_SET(PFCH_CFG_3_VAR_DESC_FL_FREE_CNT_TH_MASK, PFCH_CFG_3_VAR_DESC_FL_FREE_CNT_TH_DFLT) |
        FIELD_SET(PFCH_CFG_3_VAR_DESC_LG_PKT_CAM_CN_TH_MASK, PFCH_CFG_3_VAR_DESC_LG_PKT_CAM_CN_TH_DFLT);
    qdma_reg_write(dev_hndl, EQDMA_PFCH_CFG_3_ADDR, reg_val);
    reg_val = qdma_reg_read(dev_hndl, EQDMA_PFCH_CFG_3_ADDR);
    qdma_log_info("%s: reg = 0x%08X val = 0x%08X\n", __func__, EQDMA_C2H_PFCH_CFG_2_ADDR, reg_val);

// MD: Define constants for prefetch configuration 4
...
#define PFCH_CFG_4_DISABLE_EVT_TIMER_DFLT 0
    ...
    // MD: Set prefetch configuration 4 register
    reg_val =
        FIELD_SET(PFCH_CFG_4_GLB_EVT_TIMER_TICK_MASK, EQDMA_PFCH_CFG_4_GLB_EVT_TIMER_TICK) |
        FIELD_SET(PFCH_CFG_4_DISABLE_GLB_EVT_TIMER_MASK, PFCH_CFG_4_DISABLE_GLB_EVT_TIMER_DFLT) |
        FIELD_SET(PFCH_CFG_4_EVT_TIMER_TICK_MASK, EQDMA_PFCH_CFG_4_EVT_TIMER_TICK) |
        FIELD_SET(PFCH_CFG_4_DISABLE_EVT_TIMER_MASK, PFCH_CFG_4_DISABLE_EVT_TIMER_DFLT);
    ...
    /* MD:*************** SET_2 *******************/
// MD: Define constants for credit coalescing configuration 1
#define C2H_CRDT_COAL_CFG_1_RSVD_1_DFLT 0
#define C2H_CRDT_COAL_CFG_1_PLD_FIFO_TH_DFLT 16
#define EQDMA_C2H_CRDT_COAL_CFG_1_TIMER_TH 16 // MD:64

    // MD: Set credit coalescing configuration 1 register
    reg_val =
        FIELD_SET(C2H_CRDT_COAL_CFG_1_RSVD_1_MASK, C2H_CRDT_COAL_CFG_1_RSVD_1_DFLT) |
        FIELD_SET(C2H_CRDT_COAL_CFG_1_PLD_FIFO_TH_MASK, C2H_CRDT_COAL_CFG_1_PLD_FIFO_TH_DFLT) |
        FIELD_SET(C2H_CRDT_COAL_CFG_1_TIMER_TH_MASK, EQDMA_C2H_CRDT_COAL_CFG_1_TIMER_TH);
...
// MD: Define constants for credit coalescing configuration 2
...
#define EQDMA_C2H_CRDT_COAL_CFG_2_CRDT_TH 96
    ...
    // MD: Set credit coalescing configuration 2 register
    reg_val =
        FIELD_SET(C2H_CRDT_COAL_CFG_2_RSVD_1_MASK, C2H_CRDT_COAL_CFG_2_RSVD_1_DFLT) |
        FIELD_SET(C2H_CRDT_COAL_CFG_2_FIFO_TH_MASK, (pfch_cache_dpth - 8)) |
        FIELD_SET(C2H_CRDT_COAL_CFG_2_RESERVED1_MASK, C2H_CRDT_COAL_CFG_2_RESERVED1_DFLT) |
        FIELD_SET(C2H_CRDT_COAL_CFG_2_NT_TH_MASK, EQDMA_C2H_CRDT_COAL_CFG_2_CRDT_TH);
    ...
    /* MD:*************** SET_3 *******************/
// MD: Define constants for RRQ PCIe throttle configuration
...
#define GLBL2_RRQ_PCIE_THROT_DAT_DFLT 20480
    ...
    // MD: Set RRQ PCIe throttle configuration register
    reg_val =
        FIELD_SET(GLBL2_RRQ_PCIE_THROT_REQ_EN_MASK, EQDMA_GLBL2_RRQ_PCIE_THROT_REQ_EN) |
        FIELD_SET(GLBL2_RRQ_PCIE_THROT_REQ_MASK, GLBL2_RRQ_PCIE_THROT_REQ_DFLT) |
        FIELD_SET(GLBL2_RRQ_PCIE_THROT_DAT_EN_MASK, GLBL2_RRQ_PCIE_THROT_DAT_EN_DFLT) |
        FIELD_SET(GLBL2_RRQ_PCIE_THROT_DAT_MASK, GLBL2_RRQ_PCIE_THROT_DAT_DFLT);
...
// MD: Define constants for RRQ AXIMM throttle configuration
...
#define GLBL2_RRQ_AXIMM_THROT_DAT_DFLT 0
    ...
    // MD: Set RRQ AXIMM throttle configuration register
    reg_val =
        FIELD_SET(GLBL2_RRQ_AXIMM_THROT_REQ_EN_MASK, GLBL2_RRQ_AXIMM_THROT_REQ_EN_DFLT) |
        FIELD_SET(GLBL2_RRQ_AXIMM_THROT_REQ_MASK, GLBL2_RRQ_AXIMM_THROT_REQ_DFLT) |
        FIELD_SET(GLBL2_RRQ_AXIMM_THROT_DAT_EN_MASK, GLBL2_RRQ_AXIMM_THROT_DAT_EN_DFLT) |
        FIELD_SET(GLBL2_RRQ_AXIMM_THROT_DAT_MASK, GLBL2_RRQ_AXIMM_THROT_DAT_DFLT);
...
// MD: Define constants for RRQ bridge throttle configuration
...
#define GLBL2_RRQ_BRG_THROT_DAT_EN_DFLT 1
    ...
    // MD: Read current RRQ PCIe throttle configuration
    reg_val = qdma_reg_read(dev_hndl, EQDMA_GLBL2_RRQ_PCIE_THROT_ADDR);
    qdma_log_info("%s: BF reg = 0x%08X val = 0x%08X\n", __func__, EQDMA_GLBL2_RRQ_PCIE_THROT_ADDR, reg_val);
    data_th = FIELD_GET(GLBL2_RRQ_PCIE_THROT_DAT_MASK, reg_val);

    // MD: Set RRQ bridge throttle configuration register
    reg_val =
        FIELD_SET(GLBL2_RRQ_BRG_THROT_REQ_EN_MASK, GLBL2_RRQ_BRG_THROT_REQ_EN_DFLT) |
        FIELD_SET(GLBL2_RRQ_BRG_THROT_REQ_MASK, GLBL2_RRQ_BRG_THROT_REQ_DFLT) |
        FIELD_SET(GLBL2_RRQ_BRG_THROT_DAT_EN_MASK, GLBL2_RRQ_BRG_THROT_DAT_EN_DFLT) |
...
/* MD:****************** SET_4 *************************/
// MD: Define constants for H2C request throttle PCIe configuration
...
#define EQDMA_H2C_REQ_THROT_PCIE_DATA_TH 24576
    ...
    // MD: Set H2C request throttle PCIe configuration register
    reg_val =
        FIELD_SET(H2C_REQ_THROT_PCIE_EN_REQ_MASK, EQDMA_H2C_REQ_THROT_PCIE_EN_REQ) |
        FIELD_SET(H2C_REQ_THROT_PCIE_MASK, EQDMA_H2C_REQ_THROT_PCIE_REQ_TH) |
        FIELD_SET(H2C_REQ_THROT_PCIE_EN_DATA_MASK, EQDMA_H2C_REQ_THROT_PCIE_EN_DATA) |
        FIELD_SET(H2C_REQ_THROT_PCIE_DATA_THRESH_MASK, EQDMA_H2C_REQ_THROT_PCIE_DATA_TH);
...
// MD: Define constants for H2C request throttle AXIMM configuration
...
#define EQDMA_H2C_REQ_THROT_AXIMM_DATA_TH 16384
    ...
    // MD: Set H2C request throttle AXIMM configuration register
    reg_val =
        FIELD_SET(H2C_REQ_THROT_AXIMM_EN_REQ_MASK, EQDMA_H2C_REQ_THROT_AXIMM_EN_REQ) |
        FIELD_SET(H2C_REQ_THROT_AXIMM_MASK, EQDMA_H2C_REQ_THROT_AXIMM_REQ_TH) |
        FIELD_SET(H2C_REQ_THROT_AXIMM_EN_DATA_MASK, EQDMA_H2C_REQ_THROT_AXIMM_EN_DATA) |
        FIELD_SET(H2C_REQ_THROT_AXIMM_DATA_THRESH_MASK, EQDMA_H2C_REQ_THROT_AXIMM_DATA_TH);
...
// MD: Define constants for H2C MM data throttle configuration
...
#define EQDMA_H2C_MM_DATA_TH GLBL2_RRQ_PCIE_THROT_DAT_DFLT
    ...
    // MD: Set H2C MM data throttle configuration register
    reg_val =
        FIELD_SET(H2C_MM_DATA_THROTTLE_RSVD_1_MASK, H2C_MM_DATA_THROTTLE_RSVD_1_DFLT) |
...
static int eqdma_indirect_reg_invalidate(void *dev_hndl,
    ...
    printk(KERN_DEBUG "Indirect register context invalidated successfully\n");
    return QDMA_SUCCESS;
}
...
static int eqdma_indirect_reg_clear(void *dev_hndl,
    ...
    printk(KERN_DEBUG "Indirect register context cleared successfully\n");
    return QDMA_SUCCESS;
}
...
static int eqdma_indirect_reg_read(void *dev_hndl, enum ind_ctxt_cmd_sel sel,
    ...
    printk(KERN_DEBUG "Reading indirect register context: sel=%d, hw_qid=%d, cnt=%d\n", sel, hw_qid, cnt);
    ...
    printk(KERN_DEBUG "Indirect register context read successfully\n");
    return QDMA_SUCCESS;
}
...
static int eqdma_indirect_reg_write(void *dev_hndl, enum ind_ctxt_cmd_sel sel,
    ...
    printk(KERN_DEBUG "Writing indirect register context: sel=%d, hw_qid=%d, cnt=%d\n", sel, hw_qid, cnt);
    ...
    printk(KERN_DEBUG "Indirect register context written successfully\n");
    return QDMA_SUCCESS;
}

/* MD:
 * eqdma_get_ip_version() - Function to retrieve the IP version of the EQDMA device.
 *
 * @dev_hndl: Device handle.
 * @is_vf: Flag indicating if the device is a Virtual Function (VF).
 * @ip_version: Pointer to store the retrieved IP version.
 *
 * Returns -QDMA_ERR_INV_PARAM if the device handle is NULL, QDMA_SUCCESS otherwise.
 */
int eqdma_get_ip_version(void *dev_hndl, uint8_t is_vf, uint32_t *ip_version) {
    uint32_t ver_reg_val = 0;
    uint32_t reg_addr = (is_vf) ? EQDMA_OFFSET_VF_VERSION : EQDMA_GLBL2_MISC_CAP_ADDR;
    ...
    printk(KERN_DEBUG "Reading IP version from register: 0x%08X\n", reg_addr);
    ver_reg_val = qdma_reg_read(dev_hndl, reg_addr);

    if (!is_vf) {
        *ip_version = FIELD_GET(EQDMA_GLBL2_IP_VERSION_MASK, ver_reg_val);
    } else {
        *ip_version = FIELD_GET(EQDMA_GLBL2_VF_IP_VERSION_MASK, ver_reg_val);
    }

    printk(KERN_DEBUG "Retrieved IP version: 0x%08X\n", *ip_version);
    return QDMA_SUCCESS;
}

/* MD:
 * eqdma_fill_sw_ctxt() - Helper function to fill software context into structure.
 *
...
static void eqdma_fill_sw_ctxt(struct qdma_descq_sw_ctxt *sw_ctxt) {
    ...
    printk(KERN_DEBUG "Filling software context structure\n");
    ...
    eqdma_sw_ctxt_entries[i++].value = (sw_ctxt->ring_bs_addr >> 32) & 0xFFFFFFFF;
    ...
    printk(KERN_DEBUG "Software context structure filled\n");
}

/* MD:
 * eqdma_fill_cmpt_ctxt() - Helper function to fill completion context into structure.
 *
...
static void eqdma_fill_cmpt_ctxt(struct qdma_descq_cmpt_ctxt *cmpt_ctxt) {
    ...
    printk(KERN_DEBUG "Filling completion context structure\n");
    ...
    eqdma_cmpt_ctxt_entries[i++].value = (uint32_t)FIELD_GET(EQDMA_COMPL_CTXT_BADDR_HIGH_L_MASK, cmpt_ctxt->bs_addr);
    eqdma_cmpt_ctxt_entries[i++].value = (uint32_t)FIELD_GET(EQDMA_COMPL_CTXT_BADDR_HIGH_H_MASK, cmpt_ctxt->bs_addr);
    ...
    eqdma_cmpt_ctxt_entries[i++].value = (uint32_t)FIELD_GET(EQDMA_COMPL_CTXT_BADDR_LOW_MASK, cmpt_ctxt->bs_addr);
    eqdma_cmpt_ctxt_entries[i++].value = cmpt_ctxt->sh_cmpt;

    printk(KERN_DEBUG "Completion context structure filled\n");
}

/* MD:
 * eqdma_fill_hw_ctxt() - Helper function to fill hardware context into structure.
 *
...
static void eqdma_fill_hw_ctxt(struct qdma_descq_hw_ctxt *hw_ctxt) {
    ...
    printk(KERN_DEBUG "Filling hardware context structure\n");
    ...
    printk(KERN_DEBUG "Hardware context structure filled\n");
}

/* MD:
 * eqdma_fill_credit_ctxt() - Helper function to fill credit context into structure.
 *
...
static void eqdma_fill_credit_ctxt(struct qdma_descq_credit_ctxt *cr_ctxt) {
    printk(KERN_DEBUG "Filling credit context structure\n");

    eqdma_credit_ctxt_entries[0].value = cr_ctxt->credit;

    printk(KERN_DEBUG "Credit context structure filled\n");
}

/* MD:
 * eqdma_fill_pfetch_ctxt() - Helper function to fill prefetch context into structure.
 *
 * @pfetch_ctxt: Pointer to the prefetch context structure to be filled.
 */
static void eqdma_fill_pfetch_ctxt(struct qdma_descq_prefetch_ctxt *pfetch_ctxt) {
    int i = 0;

    printk(KERN_DEBUG "Filling prefetch context structure\n");
    ...
    printk(KERN_DEBUG "Prefetch context structure filled\n");
}

/* MD:
 * eqdma_fill_fmap_ctxt() - Helper function to fill fmap context into structure.
 *
...
static void eqdma_fill_fmap_ctxt(struct qdma_fmap_cfg *fmap_ctxt) {
    printk(KERN_DEBUG "Filling fmap context structure\n");

    eqdma_fmap_ctxt_entries[0].value = fmap_ctxt->qbase;
    eqdma_fmap_ctxt_entries[1].value = fmap_ctxt->qmax;

    printk(KERN_DEBUG "Fmap context structure filled\n");
}

/* MD:
 * eqdma_fill_intr_ctxt() - Helper function to fill interrupt context into structure.
 *
...
static void eqdma_fill_intr_ctxt(struct qdma_indirect_intr_ctxt *intr_ctxt) {
    ...
    printk(KERN_DEBUG "Filling interrupt context structure\n");
    ...
    eqdma_ind_intr_ctxt_entries[i++].value = intr_ctxt->baddr_4k & 0xFFFFFFFF;
    eqdma_ind_intr_ctxt_entries[i++].value = (intr_ctxt->baddr_4k >> 32) & 0xFFFFFFFF;
    ...
    printk(KERN_DEBUG "Interrupt context structure filled\n");
}
...
int eqdma_set_default_global_csr(void *dev_hndl) {
    ...
    printk(KERN_DEBUG "Retrieving device attributes\n");
    eqdma_get_device_attributes(dev_hndl, &dev_cap);

    printk(KERN_DEBUG "Getting IP version\n");
    rv = eqdma_get_ip_version(dev_hndl, 0, &eqdma_ip_version);
    if (rv != QDMA_SUCCESS)
        return rv;

    /* MD: Configuring CSR registers */
    printk(KERN_DEBUG "Configuring CSR registers\n");
    ...
    printk(KERN_DEBUG "Global CSR registers set to default values\n");
    return QDMA_SUCCESS;
}

/* MD:
 * dump_eqdma_context() - Helper function to dump queue context into a string.
 *
 * @queue_context: Pointer to the queue context structure.
 * @st: Status flag.
 * @q_type: Queue type.
 * @buf: Buffer to store the context dump.
...
static int dump_eqdma_context(struct qdma_descq_context *queue_context,
    ...
    printk(KERN_DEBUG "Dumping EQDMA context for queue type: %d\n", q_type);
    ...
    printk(KERN_DEBUG "EQDMA context dump completed, length: %d\n", len);
    return len;
...
/* MD:
 * dump_eqdma_intr_context() - Helper function to dump interrupt context into a string.
 *
...
static int dump_eqdma_intr_context(struct qdma_indirect_intr_ctxt *intr_ctx,
    ...
    printk(KERN_DEBUG "Filling interrupt context for ring index: %d\n", ring_index);
    ...
    printk(KERN_DEBUG "Interrupt context dump completed, length: %d\n", len);
    return len;
...
int eqdma_get_version(void *dev_hndl, uint8_t is_vf,
    ...
    printk(KERN_DEBUG "Reading EQDMA version from register: 0x%08X\n", reg_addr);
    reg_val = qdma_reg_read(dev_hndl, reg_addr);

    qdma_fetch_version_details(dev_hndl, is_vf, reg_val, version_info);

    printk(KERN_DEBUG "EQDMA version retrieved successfully\n");
    return QDMA_SUCCESS;
}

/* MD:****************************************************************************/
/* MD:*
 * eqdma_sw_context_write() - Create software context and program it.
 *
 * @dev_hndl: Device handle.
...
static int eqdma_sw_context_write(void *dev_hndl, uint8_t c2h,
    ...
    printk(KERN_DEBUG "Creating software context for hw_qid: %d\n", hw_qid);

    pasid_l = FIELD_GET(EQDMA_SW_CTXT_PASID_GET_L_MASK, ctxt->pasid);
    pasid_h = FIELD_GET(EQDMA_SW_CTXT_PASID_GET_H_MASK, ctxt->pasid);
    ...
    printk(KERN_DEBUG "Programming software context for hw_qid: %d\n", hw_qid);
...
/* MD:****************************************************************************/
/* MD:*
 * eqdma_sw_context_read() - Read software context.
 *
 * @dev_hndl: Device handle.
 * @c2h: Flag indicating if the queue is C2H (1 for C2H, 0 for H2C).
 * @hw_qid: Hardware queue ID of the queue.
...
static int eqdma_sw_context_read(void *dev_hndl, uint8_t c2h,
    ...
    printk(KERN_DEBUG "Reading software context for hw_qid: %d\n", hw_qid);
    ...
    printk(KERN_DEBUG "Software context read successfully for hw_qid: %d\n", hw_qid);
    return QDMA_SUCCESS;
}

/* MD:****************************************************************************/
/* MD:*
 * eqdma_sw_context_clear() - Clear software context.
 *
 * @dev_hndl: Device handle.
 * @c2h: Flag indicating if the queue is C2H (1 for C2H, 0 for H2C).
 * @hw_qid: Hardware queue ID of the queue.
...
static int eqdma_sw_context_clear(void *dev_hndl, uint8_t c2h,
    ...
    printk(KERN_DEBUG "Clearing software context for hw_qid: %d\n", hw_qid);
...
/* MD:****************************************************************************/
/* MD:*
 * eqdma_sw_context_invalidate() - Invalidate software context.
 *
 * @dev_hndl: Device handle.
 * @c2h: Flag indicating if the queue is C2H (1 for C2H, 0 for H2C).
 * @hw_qid: Hardware queue ID of the queue.
...
static int eqdma_sw_context_invalidate(void *dev_hndl, uint8_t c2h,
    ...
    printk(KERN_DEBUG "Invalidating software context for hw_qid: %d\n", hw_qid);
    return eqdma_indirect_reg_invalidate(dev_hndl, sel, hw_qid);
}

/* MD:****************************************************************************/
/* MD:*
 * eqdma_sw_ctx_conf() - Configure software context.
 *
 * @dev_hndl: Device handle.
 * @c2h: Flag indicating if the queue is C2H (1 for C2H, 0 for H2C).
 * @hw_qid: Hardware queue ID of the queue.
...
int eqdma_sw_ctx_conf(void *dev_hndl, uint8_t c2h, uint16_t hw_qid,
    ...
    printk(KERN_DEBUG "Configuring software context for hw_qid: %d, access_type: %d\n", hw_qid, access_type);
...
static int eqdma_pfetch_context_write(void *dev_hndl, uint16_t hw_qid,
    ...
    printk(KERN_DEBUG "Creating prefetch context for hw_qid: %d\n", hw_qid);
    ...
    printk(KERN_DEBUG "Programming prefetch context for hw_qid: %d\n", hw_qid);
    return eqdma_indirect_reg_write(dev_hndl, sel, hw_qid,
                                    pfetch_ctxt, num_words_count);
}
...
static int eqdma_pfetch_context_read(void *dev_hndl, uint16_t hw_qid,
    ...
    printk(KERN_DEBUG "Reading prefetch context for hw_qid: %d\n", hw_qid);
    ...
    /* MD: Extract fields from the read context */
    ...
    printk(KERN_DEBUG "Prefetch context read successfully for hw_qid: %d\n", hw_qid);
    return QDMA_SUCCESS;
}
...
static int eqdma_pfetch_context_clear(void *dev_hndl, uint16_t hw_qid) {
    ...
    printk(KERN_DEBUG "Clearing prefetch context for hw_qid: %d\n", hw_qid);
    return eqdma_indirect_reg_clear(dev_hndl, sel, hw_qid);
}
...
static int eqdma_pfetch_context_invalidate(void *dev_hndl, uint16_t hw_qid) {
    ...
    printk(KERN_DEBUG "Invalidating prefetch context for hw_qid: %d\n", hw_qid);
    return eqdma_indirect_reg_invalidate(dev_hndl, sel, hw_qid);
}
...
int eqdma_pfetch_ctx_conf(void *dev_hndl, uint16_t hw_qid,
    ...
    printk(KERN_DEBUG "Configuring prefetch context for hw_qid: %d with access_type: %d\n", hw_qid, access_type);
    ...
    printk(KERN_DEBUG "Prefetch context configuration completed for hw_qid: %d\n", hw_qid);
    return rv;
}
...
static int eqdma_cmpt_context_write(void *dev_hndl, uint16_t hw_qid,
    ...
    printk(KERN_DEBUG "Creating completion context for hw_qid: %d\n", hw_qid);

    baddr4_high_l = (uint32_t)FIELD_GET(EQDMA_COMPL_CTXT_BADDR_HIGH_L_MASK, ctxt->bs_addr);
    baddr4_high_h = (uint32_t)FIELD_GET(EQDMA_COMPL_CTXT_BADDR_HIGH_H_MASK, ctxt->bs_addr);
    baddr4_low = (uint32_t)FIELD_GET(EQDMA_COMPL_CTXT_BADDR_LOW_MASK, ctxt->bs_addr);

    pidx_l = FIELD_GET(QDMA_COMPL_CTXT_PIDX_GET_L_MASK, ctxt->pidx);
    pidx_h = FIELD_GET(QDMA_COMPL_CTXT_PIDX_GET_H_MASK, ctxt->pidx);

    pasid_l = FIELD_GET(EQDMA_CMPL_CTXT_PASID_GET_L_MASK, ctxt->pasid);
    pasid_h = FIELD_GET(EQDMA_CMPL_CTXT_PASID_GET_H_MASK, ctxt->pasid);
    ...
    printk(KERN_DEBUG "Programming completion context for hw_qid: %d\n", hw_qid);
    return eqdma_indirect_reg_write(dev_hndl, sel, hw_qid, cmpt_ctxt, num_words_count);
}
...
static int eqdma_cmpt_context_read(void *dev_hndl, uint16_t hw_qid,
    ...
    printk(KERN_DEBUG "Reading completion context for hw_qid: %d\n", hw_qid);
    ...
    printk(KERN_DEBUG "Completion context read successfully for hw_qid: %d\n", hw_qid);
    return QDMA_SUCCESS;
}
...
static int eqdma_cmpt_context_clear(void *dev_hndl, uint16_t hw_qid) {
    ...
    printk(KERN_DEBUG "Clearing completion context for hw_qid: %d\n", hw_qid);
    return eqdma_indirect_reg_clear(dev_hndl, sel, hw_qid);
}
...
static int eqdma_cmpt_context_invalidate(void *dev_hndl, uint16_t hw_qid) {
    ...
    printk(KERN_DEBUG "Invalidating completion context for hw_qid: %d\n", hw_qid);
    return eqdma_indirect_reg_invalidate(dev_hndl, sel, hw_qid);
}
...
int eqdma_cmpt_ctx_conf(void *dev_hndl, uint16_t hw_qid,
    ...
    printk(KERN_DEBUG "Configuring completion context for hw_qid: %d, access_type: %d\n", hw_qid, access_type);
...
static int eqdma_hw_context_read(void *dev_hndl, uint8_t c2h,
    ...
    printk(KERN_DEBUG "Reading hardware context for hw_qid: %d\n", hw_qid);
    ...
    printk(KERN_DEBUG "Hardware context read successfully for hw_qid: %d\n", hw_qid);
    return QDMA_SUCCESS;
}
...
static int eqdma_hw_context_clear(void *dev_hndl, uint8_t c2h,
    ...
    printk(KERN_DEBUG "Clearing hardware context for hw_qid: %d\n", hw_qid);
    return eqdma_indirect_reg_clear(dev_hndl, sel, hw_qid);
}
...
static int eqdma_hw_context_invalidate(void *dev_hndl, uint8_t c2h,
    ...
    printk(KERN_DEBUG "Invalidating hardware context for hw_qid: %d\n", hw_qid);
    return eqdma_indirect_reg_invalidate(dev_hndl, sel, hw_qid);
}
...
/* MD:****************************************************************************/
/* MD:*
 * eqdma_hw_ctx_conf() - Configure hardware context.
 *
 * @dev_hndl: Device handle.
 * @c2h: Flag indicating if the queue is C2H (1 for C2H, 0 for H2C).
 * @hw_qid: Hardware queue ID of the queue.
...
int eqdma_hw_ctx_conf(void *dev_hndl, uint8_t c2h, uint16_t hw_qid,
    ...
    /* MD: Context requires only H2C-0 or C2H-1, return error for any other values */
    if (c2h > 1) {
    ...
    printk(KERN_DEBUG "Configuring hardware context for hw_qid: %d\n", hw_qid);
    ...
    printk(KERN_DEBUG "Hardware context configuration completed for hw_qid: %d\n", hw_qid);
    return rv;
}
...
static int eqdma_credit_context_read(void *dev_hndl, uint8_t c2h,
    ...
    printk(KERN_DEBUG "Reading credit context for hw_qid: %d\n", hw_qid);
    ...
    printk(KERN_DEBUG "Credit context read successfully for hw_qid: %d\n", hw_qid);
    return QDMA_SUCCESS;
}
...
static int eqdma_credit_context_clear(void *dev_hndl, uint8_t c2h,
    ...
    printk(KERN_DEBUG "Clearing credit context for hw_qid: %d\n", hw_qid);
    return eqdma_indirect_reg_clear(dev_hndl, sel, hw_qid);
}
...
static int eqdma_credit_context_invalidate(void *dev_hndl, uint8_t c2h,
    ...
    printk(KERN_DEBUG "Invalidating credit context for hw_qid: %d\n", hw_qid);
    return eqdma_indirect_reg_invalidate(dev_hndl, sel, hw_qid);
}
...
int eqdma_credit_ctx_conf(void *dev_hndl, uint8_t c2h,
    ...
    printk(KERN_DEBUG "Configuring credit context for hw_qid: %d\n", hw_qid);
    ...
    printk(KERN_DEBUG "Credit context configuration completed for hw_qid: %d\n", hw_qid);
    return rv;
}
...
static int eqdma_fmap_context_write(void *dev_hndl, uint16_t func_id,
    ...
    printk(KERN_DEBUG "Writing fmap context for func_id: %hu\n", func_id);
    ...
    printk(KERN_DEBUG "Fmap context written successfully for func_id: %hu\n", func_id);
...
static int eqdma_fmap_context_read(void *dev_hndl, uint16_t func_id,
    ...
    printk(KERN_DEBUG "Reading fmap context for func_id: %d\n", func_id);
    ...
    printk(KERN_DEBUG "Fmap context read successfully for func_id: %d\n", func_id);
    return QDMA_SUCCESS;
}
...
static int eqdma_fmap_context_clear(void *dev_hndl, uint16_t func_id) {
    ...
    printk(KERN_DEBUG "Clearing fmap context for func_id: %d\n", func_id);
...
int eqdma_fmap_conf(void *dev_hndl, uint16_t func_id,
    ...
    printk(KERN_DEBUG "Configuring fmap context for func_id: %d\n", func_id);
    ...
    printk(KERN_DEBUG "Fmap context configuration completed for func_id: %d\n", func_id);
    return rv;
}
...
static int eqdma_indirect_intr_context_write(void *dev_hndl,
    ...
    printk(KERN_DEBUG "Creating indirect interrupt context for ring_index: %d\n", ring_index);
    ...
    pasid_l = FIELD_GET(EQDMA_INTR_CTXT_PASID_GET_L_MASK, ctxt->pasid);
    pasid_h = FIELD_GET(EQDMA_INTR_CTXT_PASID_GET_H_MASK, ctxt->pasid);
    ...
    printk(KERN_DEBUG "Programming indirect interrupt context for ring_index: %d\n", ring_index);
...
static int eqdma_indirect_intr_context_read(void *dev_hndl,
    ...
    printk(KERN_DEBUG "Reading indirect interrupt context for ring index: %d\n", ring_index);
    ...
    ctxt->pasid = FIELD_SET(EQDMA_INTR_CTXT_PASID_GET_L_MASK, pasid_l) |
                  FIELD_SET(EQDMA_INTR_CTXT_PASID_GET_H_MASK, pasid_h);

    printk(KERN_DEBUG "Indirect interrupt context read successfully for ring index: %d\n", ring_index);
    return QDMA_SUCCESS;
}
...
static int eqdma_indirect_intr_context_clear(void *dev_hndl,
    ...
    printk(KERN_DEBUG "Clearing indirect interrupt context for ring index: %d\n", ring_index);
    return eqdma_indirect_reg_clear(dev_hndl, sel, ring_index);
}

/* MD:****************************************************************************/
/* MD:*
 * eqdma_indirect_intr_context_invalidate() - Invalidate indirect interrupt context.
 *
 * @dev_hndl: Device handle.
 * @ring_index: Indirect interrupt ring index.
 *
...
static int eqdma_indirect_intr_context_invalidate(void *dev_hndl,
    ...
    printk(KERN_DEBUG "Invalidating indirect interrupt context for ring index: %d\n", ring_index);
...
int eqdma_indirect_intr_ctx_conf(void *dev_hndl, uint16_t ring_index,
    ...
    printk(KERN_DEBUG "Configuring indirect interrupt context for ring index: %d with access type: %d\n", ring_index, access_type);

    switch (access_type) {
    case QDMA_HW_ACCESS_READ:
        rv = eqdma_indirect_intr_context_read(dev_hndl, ring_index, ctxt);
        break;
    case QDMA_HW_ACCESS_WRITE:
        rv = eqdma_indirect_intr_context_write(dev_hndl, ring_index, ctxt);
        break;
    case QDMA_HW_ACCESS_CLEAR:
        rv = eqdma_indirect_intr_context_clear(dev_hndl, ring_index);
        break;
    case QDMA_HW_ACCESS_INVALIDATE:
        rv = eqdma_indirect_intr_context_invalidate(dev_hndl, ring_index);
        break;
    default:
        qdma_log_error("%s: access_type=%d is invalid, err:%d\n",
...
/* MD:****************************************************************************/
/* MD:*
 * eqdma_dump_config_regs() - Function to get QDMA config register dump in a buffer.
 *
 * @dev_hndl: Device handle.
 * @is_vf: Flag indicating if the device is a Virtual Function (VF).
 ...
 *
 * Returns: Length up-till the buffer is filled on success, and < 0 on failure.
 *****************************************************************************/
int eqdma_dump_config_regs(void *dev_hndl, uint8_t is_vf,
    ...
    printk(KERN_DEBUG "Dumping QDMA config registers\n");
    eqdma_get_device_attributes(dev_hndl, &dev_cap);
    ...
    for (i = 0; i < num_regs; i++) {
        ...
        /* MD: If Debug Mode not enabled and the current register
    ...
    printk(KERN_DEBUG "QDMA config register dump completed, length: %d\n", len);
    return len;
}
...
/* MD:****************************************************************************/
/* MD:*
 * eqdma_dump_queue_context() - Function to get QDMA queue context dump in a buffer.
 *
 * @dev_hndl: Device handle.
 * @st: Queue Mode (ST or MM).
 * @q_type: Queue type (H2C/C2H/CMPT).
 * @ctxt_data: Queue Context.
 * @buf: Pointer to buffer to be filled.
 * @buflen: Length of the buffer.
 *
 * Returns: Length up-till the buffer is filled on success, and < 0 on failure.
...
int eqdma_dump_queue_context(void *dev_hndl,
    ...
    printk(KERN_DEBUG "Calculating required buffer length for queue context\n");
    ...
    printk(KERN_DEBUG "Dumping queue context into buffer\n");
    rv = dump_eqdma_context(ctxt_data, st, q_type, buf, buflen);
...
/* MD:****************************************************************************/
/* MD:*
 * eqdma_dump_intr_context() - Function to get QDMA interrupt context dump in a buffer.
 *
 * @dev_hndl: Device handle.
...
int eqdma_dump_intr_context(void *dev_hndl,
    ...
    printk(KERN_DEBUG "Calculating required buffer length for interrupt context\n");
    ...
    printk(KERN_DEBUG "Dumping interrupt context into buffer\n");
...
/* MD:****************************************************************************/
/* MD:*
 * eqdma_read_dump_queue_context() - Function to read and dump the queue context in a buffer.
 *
 * @dev_hndl: Device handle.
 * @func_id: Function ID.
 * @hw_qid: Queue ID.
...
int eqdma_read_dump_queue_context(void *dev_hndl,
    ...
    printk(KERN_DEBUG "Initializing context structure\n");
    qdma_memset(&context, 0, sizeof(struct qdma_descq_context));

    if (q_type != QDMA_DEV_Q_TYPE_CMPT) {
        printk(KERN_DEBUG "Reading software context\n");
        ...
        printk(KERN_DEBUG "Reading hardware context\n");
        ...
        printk(KERN_DEBUG "Reading credit context\n");
        ...
        if (st && (q_type == QDMA_DEV_Q_TYPE_C2H)) {
            printk(KERN_DEBUG "Reading prefetch context\n");
    ...
    if ((st && (q_type == QDMA_DEV_Q_TYPE_C2H)) ||
        ...
        printk(KERN_DEBUG "Reading completion context\n");
    ...
    printk(KERN_DEBUG "Reading fmap context\n");
    ...
    printk(KERN_DEBUG "Dumping queue context into buffer\n");
    rv = dump_eqdma_context(&context, st, q_type, buf, buflen);
...
int eqdma_get_user_bar(void *dev_hndl, uint8_t is_vf,
    ...
    printk(KERN_DEBUG "Reading user bar ID from register: 0x%08X\n", reg_addr);
    ...
    printk(KERN_DEBUG "User bar found: %d\n", *user_bar);
    return QDMA_SUCCESS;
}
 ...
 *****************************************************************************/
static void eqdma_hw_ram_sbe_err_process(void *dev_hndl) {
    printk(KERN_DEBUG "Dumping SBE error debug information\n");
    eqdma_dump_reg_info(dev_hndl, EQDMA_RAM_SBE_STS_A_ADDR, 1, NULL, 0);
    eqdma_dump_reg_info(dev_hndl, EQDMA_RAM_SBE_STS_1_A_ADDR, 1, NULL, 0);
}
 ...
 *****************************************************************************/
static void eqdma_hw_ram_dbe_err_process(void *dev_hndl) {
    printk(KERN_DEBUG "Dumping DBE error debug information\n");
    eqdma_dump_reg_info(dev_hndl, EQDMA_RAM_DBE_STS_A_ADDR, 1, NULL, 0);
    eqdma_dump_reg_info(dev_hndl, EQDMA_RAM_DBE_STS_1_A_ADDR, 1, NULL, 0);
}
...
static void eqdma_hw_desc_err_process(void *dev_hndl) {
    ...
    printk(KERN_DEBUG "Dumping Descriptor Error information\n");
    for (i = 0; i < desc_err_num_regs; i++) {
        eqdma_dump_reg_info(dev_hndl, desc_err_reg_list[i], 1, NULL, 0);
    }
}
...
static void eqdma_hw_trq_err_process(void *dev_hndl) {
    ...
    printk(KERN_DEBUG "Dumping Target Access Error information\n");
    for (i = 0; i < trq_err_reg_num_regs; i++) {
        eqdma_dump_reg_info(dev_hndl, trq_err_reg_list[i], 1, NULL, 0);
    }
}
...
/* MD:****************************************************************************/
/* MD:*
 * eqdma_hw_err_dump_ctxt_info() - Dump the important context fields on HW error.
 *
 * @dev_hndl: Device handle.
 * @first_err_qid_reg: Register address for the first error QID.
 * @en_st: Flag indicating if ST Mode is enabled.
 * @c2h: Flag indicating if the mode is C2H (1 for C2H, 0 for H2C).
 *
 * Returns: void
 *****************************************************************************/
static void eqdma_hw_err_dump_ctxt_info(void *dev_hndl,
    ...
    printk(KERN_DEBUG "Reading first error QID from register: 0x%08X\n", first_err_qid_reg);
...
static void eqdma_hw_st_h2c_err_process(void *dev_hndl) {
    ...
    printk(KERN_DEBUG "Processing H2C error registers\n");
...
/* MD:****************************************************************************/
/* MD:*
 * eqdma_hw_st_c2h_err_process() - Function to dump MM C2H Error information.
 *
 * @dev_hndl: Device handle.
 *
 * Returns: void
...
static void eqdma_hw_st_c2h_err_process(void *dev_hndl) {
    ...
    printk(KERN_DEBUG "Processing C2H error registers\n");
...
/* MD:****************************************************************************/
/* MD:*
 * eqdma_mm_c2h0_err_process() - Function to dump MM C2H Error information.
 *
 * @dev_hndl: Device handle.
 *
 * This function iterates over a list of MM C2H error registers, dumping their
 * information, and then dumps the context information for MM C2H errors.
 *
 * Return: void
 *****************************************************************************/
static void eqdma_mm_c2h0_err_process(void *dev_hndl) {
    ...
    printk(KERN_DEBUG "Processing MM C2H error for device handle: %p\n", dev_hndl);

    for (i = 0; i < mm_c2h_err_num_regs; i++) {
        eqdma_dump_reg_info(dev_hndl, mm_c2h_err_reg_list[i], 1, NULL, 0);
    }

    eqdma_hw_err_dump_ctxt_info(dev_hndl, EQDMA_C2H_MM_ERR_INFO_ADDR, 0, 1);
}

/* MD:****************************************************************************/
/* MD:*
 * eqdma_mm_h2c0_err_process() - Function to dump MM H2C Error information.
 *
 * @dev_hndl: Device handle.
 *
 * This function iterates over a list of MM H2C error registers, dumping their
 * information, and then dumps the context information for MM H2C errors.
 *
 * Return: void
 *****************************************************************************/
static void eqdma_mm_h2c0_err_process(void *dev_hndl) {
    ...
    printk(KERN_DEBUG "Processing MM H2C error for device handle: %p\n", dev_hndl);

    for (i = 0; i < mm_h2c_err_num_regs; i++) {
        eqdma_dump_reg_info(dev_hndl, mm_h2c_err_reg_list[i], 1, NULL, 0);
    }

    eqdma_hw_err_dump_ctxt_info(dev_hndl, EQDMA_H2C_MM_ERR_INFO_ADDR, 0, 1);
}
 ...
 * If the index is invalid, it logs an error and returns NULL.
 *
 * Return: String representing the error name, or NULL on failure.
 *****************************************************************************/
...
/* MD:****************************************************************************/
/* MD:*
 * eqdma_hw_error_process() - Function to find the error that got triggered
 * and call the handler qdma_hw_error_handler of that particular error.
 *
 * @dev_hndl: Device handle.
 *
 * This function reads the global error status register, checks for errors,
 * and processes each error by calling the appropriate handler.
 *
...
int eqdma_hw_error_process(void *dev_hndl) {
    ...
    printk(KERN_DEBUG "Processing hardware errors for device handle: %p\n", dev_hndl);
    ...
    printk(KERN_DEBUG "Hardware error processing completed for device handle: %p\n", dev_hndl);
    return QDMA_SUCCESS;
}
...
/* MD:****************************************************************************/
/* MD:*
 * eqdma_hw_error_enable() - Function to enable all or a specific error.
 *
 * @dev_hndl: Device handle.
 * @err_idx: Error index.
 *
...
int eqdma_hw_error_enable(void *dev_hndl, uint32_t err_idx) {
    ...
    printk(KERN_DEBUG "Enabling hardware error for err_idx: %d\n", err_idx);
    eqdma_get_device_attributes(dev_hndl, &dev_cap);
    ...
    if (err_idx == EQDMA_ERRS_ALL) {
        for (i = 0; i < EQDMA_TOTAL_LEAF_ERROR_AGGREGATORS; i++) {
            ...
            /* MD: Don't access streaming registers in MM only bitstreams */
            if (!dev_cap.st_en) {
        ...
        /* MD: Don't access streaming registers in MM only bitstreams
         * QDMA_C2H_ERR_MTY_MISMATCH to QDMA_H2C_ERR_ALL are all ST errors
         */
        if (!dev_cap.st_en) {
    ...
    printk(KERN_DEBUG "Hardware error enabled successfully for err_idx: %d\n", err_idx);
    return QDMA_SUCCESS;
}

/* MD:****************************************************************************/
/* MD:*
 * eqdma_get_device_attributes() - Function to get the QDMA device attributes.
 *
 * @dev_hndl: Device handle.
...
int eqdma_get_device_attributes(void *dev_hndl,
    ...
    /* MD: Number of MM channels */
    /* MD: TODO: Register not yet defined for this. Hard coding it to 1. */
    ...
    printk(KERN_DEBUG "Device attributes retrieved successfully\n");
    return QDMA_SUCCESS;
}
...
int eqdma_init_ctxt_memory(void *dev_hndl) {
    ...
    printk(KERN_DEBUG "Initializing context memory\n");
...
#endif
    printk(KERN_DEBUG "Context memory initialized successfully\n");
    return QDMA_SUCCESS;
}

/* MD:****************************************************************************/
/* MD:*
 * get_reg_entry() - Helper function to get the register entry.
 *
...
/* MD:****************************************************************************/
/* MD:*
 * eqdma_dump_config_reg_list() - Dump the configuration registers.
 *
 * @dev_hndl: Device handle.
...
int eqdma_dump_config_reg_list(void *dev_hndl, uint32_t total_regs,
    ...
    printk(KERN_DEBUG "Dumping configuration register list\n");

    eqdma_get_device_attributes(dev_hndl, &dev_cap);

    for (reg_count = 0; (reg_count < total_regs);) {

        /* MD: Skip reading debug registers if Debug Mode is not enabled */
        if (dev_cap.debug_mode == 0 &&
    ...
    printk(KERN_DEBUG "Configuration register list dumped successfully\n");
    return len;
}
...
/* MD:****************************************************************************/
/* MD:*
 * eqdma_read_reg_list() - Read the register values.
 *
 * @dev_hndl: Device handle.
 * @is_vf: Flag indicating if the device is a Virtual Function (VF).
 * @reg_rd_group: Register read group identifier.
 * @total_regs: Pointer to store the total number of registers read.
 * @reg_list: Array of register addresses and values.
 *
 * Returns: Platform-specific error code.
 *****************************************************************************/
int eqdma_read_reg_list(void *dev_hndl, uint8_t is_vf,
    ...
    /* MD: Check if the operation is supported for PF */
    if (!is_vf) {
    ...
    /* MD: Determine the starting address based on the register read group */
    switch (reg_rd_group) {
    ...
    printk(KERN_DEBUG "Getting register entry for start address: 0x%08X\n", reg_start_addr);
    ...
    *total_regs = reg_count;
    printk(KERN_DEBUG "Total registers read: %d\n", reg_count);
    return rv;
}

/* MD:****************************************************************************/
/* MD:*
 * eqdma_write_global_ring_sizes() - Set the global ring size array.
 *
 * @dev_hndl: Device handle.
...
static int eqdma_write_global_ring_sizes(void *dev_hndl, uint8_t index,
    ...
    printk(KERN_DEBUG "Writing global ring sizes starting at index: %d\n", index);
...
/* MD:****************************************************************************/
/* MD:*
 * eqdma_read_global_ring_sizes() - Get the global ring size array.
 *
 * @dev_hndl: Device handle.
...
static int eqdma_read_global_ring_sizes(void *dev_hndl, uint8_t index,
    ...
    printk(KERN_DEBUG "Reading global ring sizes starting at index: %d\n", index);
...
static int eqdma_write_global_timer_count(void *dev_hndl, uint8_t index,
    ...
    printk(KERN_DEBUG "Setting global timer count starting at index: %u, count: %u\n", index, count);
    ...
    printk(KERN_DEBUG "Global timer count set successfully\n");
    return QDMA_SUCCESS;
}
...
static int eqdma_read_global_timer_count(void *dev_hndl, uint8_t index,
    ...
    printk(KERN_DEBUG "Reading global timer count starting at index: %u, count: %u\n", index, count);
    ...
    printk(KERN_DEBUG "Global timer count read successfully\n");
    return QDMA_SUCCESS;
}

/* MD:****************************************************************************/
/* MD:*
 * eqdma_write_global_counter_threshold() - Function to set the counter threshold values.
 *
 * @dev_hndl: Device handle.
 * @index: Index from where the values need to be written.
 * @count: Number of entries to be written.
...
static int eqdma_write_global_counter_threshold(void *dev_hndl, uint8_t index,
    ...
    printk(KERN_DEBUG "Setting global counter threshold starting at index: %u, count: %u\n", index, count);
    ...
    printk(KERN_DEBUG "Global counter threshold set successfully\n");
    return QDMA_SUCCESS;
}

/* MD:****************************************************************************/
/* MD:*
 * eqdma_read_global_counter_threshold() - Function to get the counter threshold values.
 *
 * @dev_hndl: Device handle.
 * @index: Index from where the values need to be read.
 * @count: Number of entries to be read.
...
static int eqdma_read_global_counter_threshold(void *dev_hndl, uint8_t index,
    ...
    printk(KERN_DEBUG "Reading global counter threshold starting at index: %u, count: %u\n", index, count);
    ...
    printk(KERN_DEBUG "Global counter threshold read successfully\n");
    return QDMA_SUCCESS;
}
...
/* MD:****************************************************************************/
/* MD:*
 * eqdma_write_global_buffer_sizes() - Function to set the global buffer sizes.
 *
 * @dev_hndl: Device handle.
 * @index: Index from where the values need to be written.
 * @count: Number of entries to be written.
...
static int eqdma_write_global_buffer_sizes(void *dev_hndl, uint8_t index,
    ...
    printk(KERN_DEBUG "Setting global buffer sizes starting at index: %u, count: %u\n", index, count);
    ...
    if (dev_cap.st_en) {
        qdma_write_csr_values(dev_hndl, EQDMA_C2H_BUF_SZ_ADDR, index,
                              count, glbl_buf_sz);
        printk(KERN_DEBUG "Global buffer sizes set successfully\n");
    } else {
...
/* MD:****************************************************************************/
/* MD:*
 * eqdma_read_global_buffer_sizes() - Function to get the global buffer sizes.
 *
 * @dev_hndl: Device handle.
 * @index: Index from where the values need to be read.
 * @count: Number of entries to be read.
...
static int eqdma_read_global_buffer_sizes(void *dev_hndl, uint8_t index,
    ...
    printk(KERN_DEBUG "Reading global buffer sizes starting at index: %u, count: %u\n", index, count);
    ...
    if (dev_cap.st_en) {
        qdma_read_csr_values(dev_hndl, EQDMA_C2H_BUF_SZ_ADDR, index,
                             count, glbl_buf_sz);
        printk(KERN_DEBUG "Global buffer sizes read successfully\n");
    } else {
...
int eqdma_global_csr_conf(void *dev_hndl, uint8_t index, uint8_t count,
    ...
    /* MD: Input arguments check */
    if (!dev_hndl || !csr_val) {
        qdma_log_error("%s: dev_handle or csr_val is NULL, err:%d\n",
                       __func__, -QDMA_ERR_INV_PARAM);
        return -QDMA_ERR_INV_PARAM;
    }

    printk(KERN_DEBUG "Configuring global CSR, type: %d, access: %d\n", csr_type, access_type);

    switch (csr_type) {
    case QDMA_CSR_RING_SZ:
        switch (access_type) {
        case QDMA_HW_ACCESS_READ:
            rv = eqdma_read_global_ring_sizes(dev_hndl, index, count, csr_val);
            break;
        case QDMA_HW_ACCESS_WRITE:
            rv = eqdma_write_global_ring_sizes(dev_hndl, index, count, csr_val);
            break;
        default:
            qdma_log_error("%s: access_type(%d) invalid, err:%d\n",
                           __func__, access_type, -QDMA_ERR_INV_PARAM);
    ...
    case QDMA_CSR_TIMER_CNT:
        switch (access_type) {
        case QDMA_HW_ACCESS_READ:
            rv = eqdma_read_global_timer_count(dev_hndl, index, count, csr_val);
            break;
        case QDMA_HW_ACCESS_WRITE:
            rv = eqdma_write_global_timer_count(dev_hndl, index, count, csr_val);
            break;
        default:
            qdma_log_error("%s: access_type(%d) invalid, err:%d\n",
                           __func__, access_type, -QDMA_ERR_INV_PARAM);
    ...
    case QDMA_CSR_CNT_TH:
        switch (access_type) {
        case QDMA_HW_ACCESS_READ:
            rv = eqdma_read_global_counter_threshold(dev_hndl, index, count, csr_val);
            break;
        case QDMA_HW_ACCESS_WRITE:
            rv = eqdma_write_global_counter_threshold(dev_hndl, index, count, csr_val);
            break;
        default:
            qdma_log_error("%s: access_type(%d) invalid, err:%d\n",
                           __func__, access_type, -QDMA_ERR_INV_PARAM);
    ...
    case QDMA_CSR_BUF_SZ:
        switch (access_type) {
        case QDMA_HW_ACCESS_READ:
            rv = eqdma_read_global_buffer_sizes(dev_hndl, index, count, csr_val);
            break;
        case QDMA_HW_ACCESS_WRITE:
            rv = eqdma_write_global_buffer_sizes(dev_hndl, index, count, csr_val);
            break;
        default:
            qdma_log_error("%s: access_type(%d) invalid, err:%d\n",
                           __func__, access_type, -QDMA_ERR_INV_PARAM);
    ...
    printk(KERN_DEBUG "Global CSR configuration completed with status: %d\n", rv);
    return rv;
}

/* MD:****************************************************************************/
/* MD:*
 * eqdma_global_writeback_interval_write() - Function to set the writeback interval.
 *
 * @dev_hndl: Device handle.
...
static int eqdma_global_writeback_interval_write(void *dev_hndl,
    ...
    printk(KERN_DEBUG "Setting writeback interval: %d\n", wb_int);
    ...
    printk(KERN_DEBUG "Writeback interval set successfully\n");
    return QDMA_SUCCESS;
}
...
/* MD:****************************************************************************/
/* MD:*
 * eqdma_global_writeback_interval_read() - Function to get the writeback interval.
 *
 * @dev_hndl: Device handle.
...
static int eqdma_global_writeback_interval_read(void *dev_hndl,
    ...
    if (dev_cap.st_en || dev_cap.mm_cmpt_en) {
        ...
        *wb_int = (enum qdma_wrb_interval)FIELD_GET(
        ...
        printk(KERN_DEBUG "Writeback interval read successfully\n");
    } else {
        qdma_log_error("%s: ST or MM cmpt not supported, err:%d\n",
                       __func__, -QDMA_ERR_HWACC_FEATURE_NOT_SUPPORTED);
        return -QDMA_ERR_HWACC_FEATURE_NOT_SUPPORTED;
...
/* MD:****************************************************************************/
/* MD:*
 * eqdma_global_writeback_interval_conf() - Function to configure the writeback interval.
 *
 * @dev_hndl: Device handle.
 * @wb_int: Pointer to the data to hold Writeback Interval.
 * @access_type: HW access type (qdma_hw_access_type enum) value.
...
int eqdma_global_writeback_interval_conf(void *dev_hndl,
    ...
    printk(KERN_DEBUG "Configuring writeback interval, access_type: %d\n", access_type);
...
int eqdma_mm_channel_conf(void *dev_hndl, uint8_t channel, uint8_t is_c2h,
    ...
    printk(KERN_DEBUG "Retrieving device attributes for MM channel configuration\n");
    eqdma_get_device_attributes(dev_hndl, &dev_cap);
    ...
    if (dev_cap.mm_en) {
        qdma_reg_write(dev_hndl,
                       ...
                       enable);
        printk(KERN_DEBUG "MM channel %d %s successfully\n", channel, enable ? "enabled" : "disabled");
    }
...
/* MD:****************************************************************************/
/* MD:*
 * eqdma_dump_reg_info() - Dump register information into a buffer or log.
 *
 * @dev_hndl: Device handle.
 * @reg_addr: Starting register address.
 * @num_regs: Number of registers to dump.
 * @buf: Buffer to store the register dump.
 * @buflen: Length of the buffer.
 *
 * Returns: Length of the string copied into the buffer, or an error code.
 *****************************************************************************/
int eqdma_dump_reg_info(void *dev_hndl, uint32_t reg_addr,
    ...
    /* MD: Iterate over the registers and dump their information */
    for (; j < num_regs_idx; j++) {
        ...
        for (k = 0; k < config_regs[j].num_bitfields; k++) {
            ...
            bitfield_name = config_regs[j].bitfields[k].field_name;
            lsb = 0;
            msb = 31;
    ...
    printk(KERN_DEBUG "Register dump completed, data length: %d\n", data_len);
    return data_len;
}
